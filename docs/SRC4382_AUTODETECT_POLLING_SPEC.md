...
| Source-loss debounce | V3.2/V3.3: one transient `0x13.RXCKR == 0` monitor sample must not clear the selected route; sustained loss must resume scanning within `1 s`. **V3.4 rev 0x88 amendment:** 6 consecutive misses (~2-3 s) are required before the route clears — `0x13.RXCKR` is a rate classifier that transiently reads 0 on re-measure windows/jitter/rate boundaries while audio passes, and the 2-sample confirm tore routes down on real-world status bursts (live 2026-06-12: L+5 in 20 min on a continuous source; `tests/sim/test_v34_autodetect_loss_debounce.py`). The held route stays applied through the confirmation window. | Prevents Auto Detect route flapping on unstable SRC4382 status samples. |
