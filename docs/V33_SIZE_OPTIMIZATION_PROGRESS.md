...
| 3 | Table-drive `poll_src4382_route_monitor` source-route matrix | Route selection is large and table-shaped; V3.2 source-select/SRC4382 tests should be parameterized for V3.3 before edits. | medium/high |
| 4 | Factor preset-table I2C apply bodies | `preset_table_apply_entry_legacy_blocking` and `preset_job_apply_i2c_entry` share flash-read/TAS-burst shape, with different timeout returns. | medium/high |
