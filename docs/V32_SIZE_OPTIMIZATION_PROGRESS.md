...
- Post-checkpoint baseline (pre-`W04`, 2026-04-23):
  ...
  - `+62` used bytes and `-68` free bytes vs `W03-R01`; drift is load-
    ...
    `uart_soft_recover_full`, `uart_wake_reconfigure_tx_only_and_resync_parser`) and rewired
    `run_wake_rail_gate_and_dsp_cold_init` to quiesce+re-arm the EUSART around wake delays,
...
- V3.1 `W08-R01` `float32_exp_limit1024_in_place` counted-loop rewrite,
...
- V3.2-specific additions: async `preset_job_*` state machine
  ...
  `diag_low_nibble_reply_burst`), no-pop flash entry helper
  (`flash_entry_mute_and_reset`), reset-cause classification
...
| V3.2 Layer 5 diagnostics (`diag_*`, `cmd 0x21`/`cmd 0x22`/`cmd 0x44`, `diag_low_nibble_reply_burst`) | `optimize locally` | New in V3.2, Tier-1 rev 0x37 surface |
| V3.2 no-pop flash entry helper (`flash_entry_mute_and_reset`) | `optimize locally` | New in V3.2, 4× `call` to `i2c_secondary_dev_write` / `timer3_blocking_delay` |
...
| `report_cmd29_status` / `fw_update_emit_bf18_status` BF-prefix emitters | `share/reuse candidate` | V3.1 W04-E06 residual; both still emit `0xBF` then `<id>` then `<val>` via separate `rcall uart_tx_byte_blocking` chains |
...
| --- | --- | --- | --- | --- | --- | ---: | --- | ---: | ---: | --- | --- | --- | --- | --- |
| W01 | W01-E01 | subagent | V3.2-new `call` sweep (14 sites in `flash_entry_mute_and_reset`, `preset_job_*`, `hid_diag_memread__read_flash_region`) | Convert reachable V3.2-new `call` targets to `rcall`; V3.1 `W06` did not cover these | yes | 15241 | `0x4BED` | 18 | -16 | source smoke 20/20 pass | parent accepted into `W01-R01` | yes | low | 8 of 14 sites converted. 6 sites reverted due to gpasm ±1024-word reach error (`flash_read` / `i2c_byte_tx` too far). |
| W01 | W01-E02 | subagent | Shared BF-prefix helper for `report_cmd29_status` + `fw_update_emit_bf18_status` | V3.1 `W04-E06` port. Both emitters share `0xBF → rcall uart_tx_byte_blocking` preamble; factor into a tiny rcall-reachable helper. | yes | 15253 | `0x4BF9` | 6 | -4 | source smoke 20/20 pass | parent accepted into `W01-R01` | yes | low | Different approach: replaced trailing `goto uart_tx_byte_blocking` with `bra` (2-byte encoded) in both emitters. Shared-helper option was net-zero after helper overhead. |
| W01 | W01-E03 | subagent | Merge `preset_job_commit_idle` and `preset_job_service__clear_state_and_return` tails | Both are `movlb 0x2; clrf preset_job_state, BANKED; return 0`. Merge one label to fall through to the other; single shared tail. | yes | 15253 | `0x4BF9` | 6 | -4 | source smoke 18/18 pass | parent accepted into `W01-R01` | yes | medium | Approach B chosen: `preset_job_commit_idle` replaced with `bra preset_job_service__clear_state_and_return`. Approach A (fall-through) not viable — labels not adjacent; intervening labels `preset_job_cancel_unmute` and `preset_job_cancel`. |
...
| W02 | W02-E01 | subagent | `eeprom_read_byte_at_w` helper (17 sites in `restore_eeprom_settings_on_boot`) | Collapse 17 × 5-instruction EEPROM read preambles into 2-instruction `movlw addr; rcall helper` form | yes | 15100 | `0x4B61` | 158 | -124 | source smoke 20/20 | parent accepted into `W02-R01` | yes | low | All 17 sites converted cleanly within ±1024-word rcall reach. Helper placed at end of restore_eeprom_settings_on_boot. Exactly matches predicted -124 B. |
| W02 | W02-E02 | subagent | `ram_block_clear_four_bytes_from_w` helper (7 sites in `i2c_apply_channel_route_sync_burst`) | Collapse 7 × 4-instruction clear_ram_span_from_staged_addr_count setups (always block size 0x04) | yes | 15194 | `0x4BBF` | 64 | -30 | source smoke 20/20 | parent accepted into `W02-R01` | yes | low | All 7 sites uniform. Helper placed immediately before i2c_apply_channel_route_sync_burst. |
| W02 | W02-E03 | subagent | `usb_hid_mailbox_stage_selector5_if_enabled` helper (4 sites) | Collapse the 6-line USB mailbox service pattern (loads 0x05 into ram_0x0C1, calls usb_hid_mailbox_send_reply_if_ready conditionally) | yes | 15192 | `0x4BBD` | 66 | -32 | source smoke 20/20 + 13/13 no-pop | parent accepted into `W02-R01` | yes | medium | 5th candidate site had different BSR setup and was correctly left inline. |
| W02 | W02-E04 | subagent | `hex_lookup_table_ptr` helper (4 sites) | Factor `addlw LOW(hex_lookup_table); movwf TBLPTRL; movlw HIGH(hex_lookup_table); movwf TBLPTRH` preamble | yes | 15212 | `0x4BD1` | 46 | -12 | source + full -k v32 | parent accepted into `W02-R01` | yes | medium | 3 sites rcall, 1 far site (hex_scratch_nibble_to_ascii) uses call due to reach. |
| W02 | W02-E05 | subagent | `setup_fsr2_page1_or_page2_from_w_carry` helper (10 sites) | Factor `movwf FSR2L; clrf FSR2H; movlw 0x01; addwfc FSR2H, F` indirect-page setup | yes | 15177 | `0x4BAD` | 82 | -47 | happy-path 16/16 | parent accepted into `W02-R01` | yes | medium | 9 sites rcall + 1 far site (5830) call. Carry-flag semantics preserved across rcall. |
...
| W02 | W02-E08 | subagent | Confirmation variant of W02-E01 | Independent re-application of the 17-site eeprom_read_byte_at_w wrapper | yes | 15100 | `0x4B61` | 158 | -124 | source smoke 20/20 | confirmation only | no | low | Matched W02-E01 exactly. Helper placed before `restore_eeprom_settings_on_boot` (different location, same result). |
...
| W03 | W03-E02 | subagent | `tas3108_write_zero_volume_coeff` fall-through helper (4 sites) | Factor 4-register clrf + call coeff_write; helper falls through into i2c_tas3108_coeff_write | yes | 14925 | `0x4AC9` | 310 | -26 | stable-file smoke green | parent accepted into `W03-R01` | yes | low | Site 4 uses bra tail-call (in-reach); sites 1–3 use call (far). Net −26 B. |
...
| W03 | W03-E04 | subagent | `setup_fsr2_page1_from_w` helper (3 sites) | Factor 4-line FSR2 page setup; per-site audit proved carry always 1, so helper sets FSR2H=1 (not 0 as spec estimate suggested) | yes | 14971 | `0x4ADF` | 288 | -4 | source+extended smoke 136 pass | parent accepted into `W03-R01` | yes | low | Used `call` not `rcall` (sites >±1024 words from helper). |
...
| drift | `9ab78d4` | n/a (feature) | Wake-path UART hardening checkpoint | Load-bearing fix (not a size experiment): 5 new helpers (`uart_fifo_drain_2`, `uart_parser_resync`, `uart_quiesce_for_wake`, `uart_soft_recover_full`, `uart_wake_reconfigure_tx_only_and_resync_parser`) + `run_wake_rail_gate_and_dsp_cold_init` wire-up + OERR recovery refactor + EEPROM revision byte bump `0x37→0x38`. | yes | 14942 | `0x4AC9` | 310 | +62 | not re-gated post-checkpoint | merged feature, not a size win | yes | n/a | Recorded here so W04 baseline lineage is traceable. Campaign reopened 2026-04-23 with `free >= 500 B` target; W04..Wn now needed to recover ≥190 B. |
| W04 | W04-E01 | subagent | `float32_pack_mantissa_exponent_sign_and_save` wrapper (3 sites) | Factor the `(r)call float32_pack_mantissa_exponent_sign + 4 movff + return 0` tail at L3988-3994, L6558-6563, L7950-7955 into a single wrapper; replace each site with `bra float32_pack_mantissa_exponent_sign_and_save`. Est ~30-34 B. | yes | 14916 | `0x4AA9` | 342 | -32 | smoke 41/41 pass | parent accepted into `W04-R01` | yes | low | Site 1 (L3988) bra-reachable (+516 instr); sites 2/3 use goto (far). Helper `float32_pack_mantissa_exponent_sign_and_save` at 0x2EC8. Largest single-experiment win of W04. |
| W04 | W04-E02 | subagent | `ram_clear_prepare_page1_address_high` helper (9 sites) | Factor `movlb 0x1; movlw 0x01; movwf ram_0x004, ACCESS` 3-instruction prefix at 9 `clear_ram_span_from_staged_addr_count`/`ram_block_clear_four_bytes_from_w` callers (L756, L764, L772, L2549, L2558, L2563, L5815, L5845, L7923). Est ~20-28 B. | yes | 14930 | `0x4AB9` | 326 | -16 | smoke 23/23 pass (coordinator re-ran test_firmware_version_label + test_v31_happy_path) | parent candidate for `W04-R01` | tbd | low | All 9 sites converted: 3 rcall (L2556, L2563, L2566) + 6 call (L756, L762, L768, L5816, L5844, L7920). Helper at L2509. |
...
| W04 | W04-E04 | subagent | `fw_update_stage_flash_addr_from_cursor` helper (3 sites) | Factor `movff ram_0x082→003; movff ram_0x083→004; clrf 005; clrf 006` flash-address preamble at L3782, L3855, L3870 inside `fw_update_commit_hid_payload_page`. Est ~10-16 B. | yes | 14926 | `0x4AB9` | 326 | -16 | smoke 40/40 pass + 2 pre-existing skips | parent candidate for `W04-R01` | tbd | low | All 3 sites converted via `rcall`. Helper placed at 0x2914 (just before `fw_update_commit_hid_payload_page`). Matches prediction exactly (3 × (12-2) − 14 helper = 16 B). BSR preserved across all sites. |
| W04 | W04-E05 | subagent | `program_uart_31250_baud_common` helper audit (3 sites, label-crossing) | Factor `clrf SPBRGH; movlw 0x7F; movwf SPBRG; bcf OSCCON, 1` 4-instruction shared prefix at L4082, L5671, L6114. Mid-sequence labels at sites 2/3 cap savings to the 4-instruction prefix only. Est ~0-10 B. | yes | 14943 | `0x4AC3` | 316 | -6 | smoke 24/24 pass | parent candidate for `W04-R01` | tbd | low | Option A (4-instr helper). Site 1 uses call, sites 2/3 use rcall. Helper at 0x38D2. Label-crossing respected: only the 4-instr prefix is factored; site-local `bcf LATB, 4` + branch-convergence label remain inline. Used_bytes +1 is code-byte-shift artifact; `free` metric is authoritative. |
| W04 | W04-E06 | subagent | Post-W03+checkpoint `call` → `rcall` sweep | Layout-shift-exposed reachable sites after W03-R01 (`-95 B`) + checkpoint (`+62 B`). Precedent: `W02-E07` picked up 2 additional conversions after `W01-R01`. Est ~4-12 B. | yes | 14926 | `0x4AB9` | 326 | -16 | smoke 44/44 pass | parent candidate for `W04-R01` | tbd | low | 8 sites converted (1 `setup_fsr2_page1_from_w` at L2454 + 7 `ram_block_clear_four_bytes_from_w` at L2540-2567). Best single-experiment post-W03 win; previous W02-E07 found 2, W04 finds 8 due to layout shift from W03-R01 (-95 B) + checkpoint (+62 B). Only 1 borderline site remains (disp +2062, just 16 B over the ±2046 B reach ceiling). |
| W04 | W04-E07 | subagent | UART hardening helper fall-through cluster | Reorder the 5 new UART helpers so one callsite (`uart_soft_recover_full` or `uart_quiesce_for_wake`) falls through into `uart_parser_resync`; also opportunistically swap `goto uart_parser_resync` → `bra uart_parser_resync` at `uart_wake_reconfigure_tx_only_and_resync_parser` / `uart_reconfigure_and_resync_parser` where in reach. Est ~2-6 B. | yes | 14942 | `0x4AC3` | 316 | -6 | smoke 37/37 pass | parent candidate for `W04-R01` | tbd | low | Reorder: `uart_fifo_drain_2` → `uart_quiesce_for_wake` → `uart_soft_recover_full` → `uart_parser_resync`; `uart_soft_recover_full` falls through into `uart_parser_resync` (2 B). `uart_wake_reconfigure_tx_only_and_resync_parser` + `uart_reconfigure_and_resync_parser` swap `goto`→`bra uart_parser_resync` (2 B × 2 = 4 B). Total 6 B. |
| W04 | W04-E08 | subagent | `timer3_blocking_delay_ms_from_w` helper audit | Factor `clrf ram_0x004; movlw <ms>; movwf ram_0x003; call timer3_blocking_delay` preamble (4 instructions) if ≥4 uniform sites exist in `run_wake_rail_gate_and_dsp_cold_init` + related wake paths. Est ~8-12 B if viable; REJECT if < 4 sites. | yes | 14925 | `0x4AB1` | 334 | -24 | smoke 24/24 pass | parent accepted into `W04-R01` | yes | low | Found 5 matching sites (L1186, L4050, L4075, L4089, L4101) — all converted. Helper `timer3_blocking_delay_ms_from_w` placed at ~0x4540. Pattern was uniform across fw_update_relay + 4 run_wake_rail_gate_and_dsp_cold_init timer3 preambles. Best ratio of lines edited to bytes saved in W04. |
| W04 | W04-R01 | `coordinator` | recombination of W04-E01 + E02 + E03 + E04 + E05 + E06 + E07 + E08 (all 8 ACCEPT) on top of checkpoint-drifted baseline | Stack all 8 wins (no null-results or rejections in W04). E02+E06 overlap on ram_block_clear_four_bytes_from_w bank-1 sites and E05+E08 overlap on adc_boot_gate__start_dsp_cold_init hunk, but both pairs stack compatibly (E02 factors preamble, E06 converts tail; E05 factors SPBRG prefix, E08 factors timer3 preamble). | yes | 14838 | `0x4A59` | 422 | -112 | full gate: `236 passed, 3 skipped, 15 xfailed, 7 failed (4 flakey + 3 pre-existing wake-path regressions from 9ab78d4 checkpoint)` | accepted baseline | yes | medium | Authoritative accepted W04 baseline, committed as `4d15c8a`. Individual parent sum was -122 B; recombination yielded -112 B (10 B overlap penalty at E02/E06 + E05/E08 intersection hunks). Helpers added: `ram_clear_prepare_page1_address_high`, `program_uart_31250_baud_common`, `float32_pack_mantissa_exponent_sign_and_save`, `fsr2_page0_read_w`, `fw_update_stage_flash_addr_from_cursor`, `timer3_blocking_delay_ms_from_w`. Gate failures are the exact 7 expected (4 known-flakey + 3 wake-path). W04-E08's factoring silently broke `tests/sim/test_v32_no_pop_flash_entry.py::test_helper_sequence_is_in_order` because the test reads from primary V32_MAIN_ASM and was previously asserting on pre-W04-E08 inline `clrf ram_0x004` phase markers — not surfaced by the W04-R01 gate because that gate ran before `4d15c8a` committed to primary. Narrowed the test's `HELPER_SEQUENCE` phase-4 patterns (collapsed 4a/b/c/d into 2 markers that accept either inline or factored form via `timer3_blocking_delay_ms_from_w`) in the same spirit as W01-R01's `call` / `rcall` narrowing. |
| W05 | W05-E01 | subagent | `i2c_381c_with_w_bank0` helper (5 sites) | Factor `movwf ram_0x013, ACCESS; call preset_table_apply_entry_legacy_blocking, 0x0; movlb 0x0` 3-instr pattern at 5 sites (L1590, L1600, L1610, L1620, L1630 in `cmd_dispatch_gated_*`). Est ~10-20 B. | yes | 14818 | `0x4A45` | 442 | -20 | smoke 44/44 pass | parent accepted into `W05-R01` | yes | low | All 5 sites converted with `rcall` (all within ±1024-word range of the helper placed after `usb_hid_mailbox_stage_selector5_if_enabled`). Matches best-case prediction exactly. |
| W05 | W05-E02 | subagent | `setup_fsr2_page2_from_w` helper (5 sites) | Factor `movwf FSR2L; movlw 0x02; movwf FSR2H` page-2 pointer setup at 5 sites (L711, L949, L2457, L5987, L7992). Est ~4-12 B. | yes | 14829 | `0x4A55` | 426 | -4 | smoke 32/32 pass | parent accepted into `W05-R01` | yes | low | 4 sites use `call` (out of rcall reach), 1 site uses `rcall`. Helper clobbers W=0x02; audited each site's next-instruction for W-sensitivity — all safe. Smaller savings than W-source-reachable E01 due to reach limits. |
...
- `W01-R01` parents (accepted 2026-04-21):
  ...
  - `W01-E02` `goto` → `bra` tail-jumps in `report_cmd29_status` /
    `fw_update_emit_bf18_status` (2 sites, −4 B)
  - `W01-E03` `preset_job_commit_idle` → `bra preset_job_service__clear_state_and_return`
    tail merge (−4 B)
...
- `W02-R01` parents (accepted 2026-04-21):
  - `W01-R01` baseline
  - `W02-E01` `eeprom_read_byte_at_w` wrapper (17 sites, −124 B)
  - `W02-E02` `ram_block_clear_four_bytes_from_w` helper (7 sites, −30 B)
  - `W02-E03` `usb_hid_mailbox_stage_selector5_if_enabled` helper (4 sites, −32 B)
  - `W02-E04` `hex_lookup_table_ptr` helper (4 sites, −12 B)
  - `W02-E05` `setup_fsr2_page1_or_page2_from_w_carry` helper (10 sites, −47 B)
...
- `W03-R01` parents (accepted 2026-04-21, final baseline):
  ...
  - `W03-E01` `nibble_to_hex_ascii_from_01B` fall-through helper
    (4 sites, −20 B)
  - `W03-E02` `tas3108_write_zero_volume_coeff` fall-through helper with
    tail-call at 4th site (4 sites, −26 B)
  - `W03-E03` `sspcon1_masked_w` helper (4 sites, −20 B)
  - `W03-E04` `setup_fsr2_page1_from_w` helper (3 sites, carry-always-1
...
- `W01-R01` accepted actions (2026-04-21):
  - converted 8 V3.2-new `call` sites to `rcall` in
    `flash_entry_mute_and_reset`, `preset_job_apply_i2c_recover`,
  ...
  - replaced trailing `goto uart_tx_byte_blocking` with `bra` in
    `report_cmd29_status` and `fw_update_emit_bf18_status` (PC-relative
    2-byte encoding)
  - replaced `preset_job_commit_idle`'s 3-instruction tail with a
    single `bra preset_job_service__clear_state_and_return` (shared tail)
...
- `W02-R01` accepted actions (2026-04-21):
  - Added `eeprom_read_byte_at_w` rcall wrapper at end of
    `restore_eeprom_settings_on_boot`; converted 17 of 17 EEPROM read preamble
    sites in that function to 2-instruction `movlw addr; rcall helper`
    form.
  - Added `ram_block_clear_four_bytes_from_w` rcall wrapper before
    `i2c_apply_channel_route_sync_burst`; converted 7 of 7 block-clear setups with
    fixed size 0x04 to 2-instruction `movlw start; rcall helper` form.
  - Added `usb_hid_mailbox_stage_selector5_if_enabled` rcall wrapper after
  ...
  - Added `hex_lookup_table_ptr` rcall wrapper after
    ...
    (`hex_scratch_nibble_to_ascii`) to `call` due to ±1024-word reach limit.
  - Added `setup_fsr2_page1_or_page2_from_w_carry` rcall wrapper in the same slot as
    `usb_hid_mailbox_stage_selector5_if_enabled` (sequential); converted 9 of 10 FSR2
...
- `W03-R01` accepted actions (2026-04-21, final):
  - Added `nibble_to_hex_ascii_from_01B` fall-through helper before
    ...
    itself.
  - Added `tas3108_write_zero_volume_coeff` fall-through helper before
  ...
  - Added `setup_fsr2_page1_from_w` helper; factors 3 FSR2 page-1-only
...
HEAD. The checkpoint commit `9ab78d4` reshaped `run_wake_rail_gate_and_dsp_cold_init` and OERR
...
- 2026-04-21: queued `W01` wave of 8 experiments targeting V3.2-new
  ...
  `flash_entry_mute_and_reset`, reset-cause classifier). Launched in
...
- 2026-04-21: first full-gate run on `W01-R01` failed 1 structural
  ...
  `flash_entry_mute_and_reset`. Relaxed the three HELPER_SEQUENCE
...
- 2026-04-21: queued `W02` wave of 8 experiments targeting higher-level
  ...
  `clear_ram_span_from_staged_addr_count` setup helper, 4-site USB mailbox service helper,
