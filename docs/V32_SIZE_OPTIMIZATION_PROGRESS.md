...
- Current accepted baseline (post-`W05-R01`, 2026-04-24, pending final gate):
  - `used_bytes_pre_preset_b=14746`
  - `last_used_pre_preset_b=0x49FF`
  - `free_bytes_before_0x4C00=512` — **target `free >= 500 B` MET**
...
| W05 | W05-E08 | subagent | goto→bra sweep + call+return→goto tail conversions | 10 safe `goto X` → `bra X` swaps + 6 `call X; return 0` → `goto/bra X` tail conversions scattered across i2c, uart, flash, USB, timer3 paths. Standalone measurement: -32 B. | yes | 14802 | `0x4A39` | 454 | -32 | smoke 108/110 pass (1 xfail preexisting, 1 skip) | **partial** accepted into `W05-R01`: ~5 of 16 rewrites landed (only the call/rcall tail-conversions — the 10 goto→bra swaps did NOT land in the recombination due to patch-apply order interactions) | partial | low | Standalone worked perfectly; recombination sequence lost the 10 goto→bra hunks when E07's `cmd_dispatch_gated_i2c_pair` edits ran first and shifted the context bands E08's goto→bra patches needed. The ~5 tail-conversions (e.g. L1432 call→goto `main_i2c_service_48e2`, L2523 call→goto `ram_block_clear`, L9022 rcall→bra `uart_tx_byte_blocking`) landed via the subset of E08 hunks that didn't touch E07's area. Net W05-R01 contribution: ~10 B, not -32 B. The 10 missed goto→bra swaps remain applicable to the W05-R01 layout and are queued for W06 as a revisit candidate. |
| W05 | W05-R01 | `coordinator` | recombination of W05-E01 + E02 + E04 + E05 + E06 + E07 + E08(partial) on top of W04-R01, committed as `325dfe9` | Stack 6 full W05 wins + partial E08 (tail-conversions only; E08's 10 goto→bra hunks were lost during patch-apply order interaction with E07's `cmd_dispatch_gated_i2c_pair` tail-merge), plus a small test-pattern narrowing for the W04-E08-introduced `test_helper_sequence_is_in_order` regression. E04+E05 overlap at `flash_read` call (E05 lost its L5550 site). | yes | 14746 | `0x49FF` | 512 | -92 | full gate `236 passed, 3 skipped, 15 xfailed, 7 failed (3 wake-path + 4 flakey-pool, exact same expected-fail set as W04-R01 baseline)` | accepted baseline | yes | medium | **Target `free >= 500 B` MET at 512 B.** Individual standalone parent sum was -124 B; W05-R01 landed -92 B (32 B gap breakdown: ~22 B from E08's 10 lost goto→bra hunks, ~8 B from E04/E05 overlap at flash_read, ~2 B from generic compaction-interaction). Helpers added: `i2c_381c_with_w_bank0`, `fsr2_page2_from_W`, `flash_read_fsr2_0017`, `core_filter_addr_from_0x0D3`. Plus tail-call conversion in `cmd_dispatch_gated_i2c_pair` and 5 E08-partial mechanical call/rcall-tail rewrites. The 10 missed E08 goto→bra swaps are queued for W06 investigation as a recombination-sequence robustness fix rather than a fresh experiment. |
