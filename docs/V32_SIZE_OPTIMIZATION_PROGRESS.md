...
| W05 | W05-E08 | subagent | goto→bra sweep + call+return→goto tail conversions | 10 safe `goto X` → `bra X` swaps + 6 `call X; return 0` → `goto/bra X` tail conversions scattered across i2c, uart, flash, USB, timer3 paths. | yes | 14802 | `0x4A39` | 454 | -32 | smoke 108/110 pass (1 xfail preexisting, 1 skip) | parent accepted into `W05-R01` (1 of 16 sites conflicts with E07) | yes | low | 16 distinct mechanical rewrites. Recombination lost 1 site (the `call main_i2c_service_48e2; return 0` at L1432 inside `cmd_dispatch_gated_i2c_pair` — E07 changed that epilogue to `bra flow_1990` so the tail-conversion no longer applies). 15 of 16 rewrites stack; net W05-R01 contribution: ~30 B. |
| W05 | W05-R01 | `coordinator` | recombination of W05-E01 + E02 + E04 + E05 + E06 + E07 + E08 (7 ACCEPT, E03 REJECT) on top of W04-R01 | Stack 7 W05 wins plus a small test-pattern narrowing for the W04-E08-introduced `test_helper_sequence_is_in_order` regression. E04+E05 overlap at `flash_read` call (E05 lost its L5550 site); E07+E08 overlap at `cmd_dispatch_gated_i2c_pair` (E08 lost its L1432 hunk). | yes | 14746 | `0x49FF` | 512 | -92 | full gate: in progress | accepted baseline (pending gate) | pending | medium | **Target `free >= 500 B` MET.** Individual parent sum was -124 B (including E05 +12 and E08 +32 with all sites), recombination yielded -92 B (32 B overlap penalty at E04/E05 and E07/E08 intersection hunks + W04 layout already compacted some of E05's gains). Helpers added: `i2c_381c_with_w_bank0`, `fsr2_page2_from_W`, `flash_read_fsr2_0017`, `core_filter_addr_from_0x0D3`. Plus tail-call conversion in `cmd_dispatch_gated_i2c_pair` and ~15 mechanical goto→bra / call-tail rewrites. |
