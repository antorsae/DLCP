...
- MAIN `V35_MAIN_HEX`: V3.5 EEPROM rev `0x0099`, SHA-256 `fcc882e9ef1ec7cd0c5923530cd7a8e4e63c893a02bf08e418b575fb0ca76e92`.
- CONTROL `V173_CONTROL_HEX`: V1.73 rev `0x60`, build `20260630`, SHA-256 `6bc5abd6a8ea9fc96360c6f99cd524ba3f84dcc425ae0d1d1cbb15505b670875`.
...
| `test_v35_chain_copy_tos_rewrite_masks_and_restores_prior_gie` | V3.5 `chain_copy` descriptor-skip TOS rewrite interrupt window | `V35_MAIN_ASM`; fixed at V3.5 rev `0x0095` and carried by current canonical rev `0x0099` | Helper must preserve prior `GIE`, clear `GIE` for the `TOSL/TOSH` commit, restore only if previously set, and leave `BSR=0` | Previous source failed the structural proof and remained strict-xfailed | Focused chain-copy/release identity gate passed; `main-v35` RAM safety OK | Full all-tests gate: `2135 passed, 21 skipped, 2 xfailed, 7 warnings in 1662.40s` | Not run; structural PIC18 contract |
| `test_source_assembled_v35_unarmed_relay_rejects_first_42_report`, `test_canonical_v35_unarmed_relay_rejects_first_42_report_after_release_build`, `test_old_relay_false_ack_behavior_reproduces_with_temp_mutation`, `test_full_chain_fixed_main_flashes_control_v173_through_real_bootloader`, and `test_full_chain_fixed_main_flashes_newer_v173_through_real_bootloader` | CONTROL flash relay can return success-looking `42 00...` while unarmed, or accept CRC while failing to program the final `0x77B0` metadata row | `V35_MAIN_HEX`, source-assembled V3.5 mutation fixture, `V173_CONTROL_HEX`, builder-generated newer V1.73 target | MAIN must return `42 12...` when unarmed; manual-bootloader full-chain flash must end `41 00 aa` and read back target application bytes/metadata | Old source mutation returns `42 00...` with session/signature clear and final `41 11...`; pre-boundary fix leaves `0x77B0..` erased despite CRC success | Fast relay/flasher gate: `32 passed`; structural relay/listing gate: `3 passed`; slow full-chain gate: `2 passed` | Full all-tests gate: `2144 passed, 21 skipped, 2 xfailed, 7 warnings in 1302.84s` | Not run; manual bootloader live flash remains field confirmation |
