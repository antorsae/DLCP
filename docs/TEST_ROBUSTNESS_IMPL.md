...
- MAIN `V35_MAIN_HEX`: V3.5 EEPROM rev `0x009B`, SHA-256 `7238d08cacf32f25358cf1a83d86984cb7c1d454ce46051bafe56acc3eed1071`.
- CONTROL `V173_CONTROL_HEX`: V1.73 rev `0x63`, build `20260702`, SHA-256 `9a28543e99ff1806a470826283323e9438a29dd6a4aa6917a27152a1631c2ee1`.
...
| `test_v173_canonical_pb1_spdif_pb2_aes_persisted_inputs_survive_cold_boot`, `test_v173_canonical_pb1_pb2_dirty_save_commits_eeprom_and_clean_save_no_churn`, invalid/erased/corrupt canonical tests, and `test_every_user_selected_concrete_pb2_source_saves_documented_encoding` | PB1/PB2 input persistence divergence, immediate-write mistakes, corrupt-byte import, ambiguous BF/06 import, PB2 full-table row-to-cmd06 mapper drift | `V173_CONTROL_HEX` + `V35_MAIN_HEX` for release cases; source-assembled V1.73 for exhaustive row mapping | CONTROL EEPROM bytes `0x5E`/`0x5F`, simulator routing state, dirty-save instrumentation, PB2 UI row -> intent -> `B2/06` -> EEPROM encoding | Would fail if PB1/PB2 wrote instantly, never persisted, repeated clean saves, imported corrupt PB2 intent, or mapped PB2 `Analogue 3` to Auto Detect | PB2 all-row mapping subset: `10 passed in 122.44s` | Full all-tests gate: `2135 passed, 21 skipped, 2 xfailed, 7 warnings in 1662.40s` | Not run; live persistence/audio gates still required for field closure |
...
| `test_v35_chain_copy_tos_rewrite_masks_and_restores_prior_gie` | V3.5 `chain_copy` descriptor-skip TOS rewrite interrupt window | `V35_MAIN_ASM`; fixed at V3.5 rev `0x0095` and carried by current canonical rev `0x009B` | Helper must preserve prior `GIE`, clear `GIE` for the `TOSL/TOSH` commit, restore only if previously set, and leave `BSR=0` | Previous source failed the structural proof and remained strict-xfailed | Focused chain-copy/release identity gate passed; `main-v35` RAM safety OK | Full simulator gate: `2175 passed, 2 skipped, 2 xfailed, 7 warnings in 1610.69s` | Not run; structural PIC18 contract |
| `test_source_assembled_v35_unarmed_relay_rejects_first_42_report`, `test_canonical_v35_unarmed_relay_rejects_first_42_report_after_release_build`, `test_old_relay_false_ack_behavior_reproduces_with_temp_mutation`, `test_full_chain_fixed_main_flashes_control_v173_through_real_bootloader`, and `test_full_chain_fixed_main_flashes_newer_v173_through_real_bootloader` | CONTROL flash relay can return success-looking `42 00...` while unarmed, accept CRC while failing to program the final `0x77B0` metadata row, or inject HID-packet-boundary CR/LF inside a downstream `:10` Intel HEX data record | `V35_MAIN_HEX`, source-assembled V3.5 mutation fixture, `V173_CONTROL_HEX`, builder-generated newer V1.73 target | MAIN must return `42 12...` when unarmed; manual-bootloader full-chain flash must end `41 00 aa`, read back target application bytes/metadata, and emit only complete routed `:10` data records | Old source mutation returns `42 00...` with session/signature clear and final `41 11...`; pre-boundary fix leaves `0x77B0..` erased despite CRC success; pre-`0x009A` packet fallthrough can split `:103060...` across CR/LF and falsely pass input-stream CRC | Fast relay/flasher gate: `32 passed`; structural relay/listing gate: `3 passed`; slow full-chain gate with complete-record assertions: `2 passed, 32 deselected in 49.37s` | Full simulator gate: `2150 passed, 2 skipped, 2 xfailed, 7 warnings in 991.13s` | Not run; manual bootloader live flash remains field confirmation |
| `test_v35_duplicate_wake_frames_preserve_pending_wake_dispatch`, `test_v35_boot_clamps_corrupt_channel6_without_mutating_channel5_or_shadows`, `test_v35_repeated_fixed_input_cmd06_does_not_rewrite_route_or_increment_c`, `test_v35_rx_ring_read_masks_and_restores_prior_gie_around_dequeue`, `test_v173_identity_parser_waits_for_v35_rev16_tail_before_valid`, and `test_v173_waiting_four_sentinel_reduce_uses_non_isr_scratch` | 2026-07-02 Fable confirmed bugs: duplicate wake cancels bring-up, channel-6 sanitizer writes channel 5, fixed-input full-sync churns route/I2C counters, RX dequeue races OERR resync, V3.5 identity commits before `BF/54..55`, cold-WAITING foreground predicate aliases ISR scratch | Source-assembled V3.5/V1.73 plus canonical `V35_MAIN_HEX` rev `0x009B` and `V173_CONTROL_HEX` rev `0x63` after rebuild | Product-visible wake/audio route, boot source RAM/shadow, fixed-input idempotence, structural interrupt safety, Diagnostics identity revision, and cold-WAITING connection safety | Pre-fix focused run failed 27 nodes, including duplicate wake `diag_b==0`, channel 6 corrupt/channel 5 clamped, fixed-input `C` counter increments, old RX critical section missing, V3.5 valid at `BF/53`, and WAITING use of `(Common_RAM + 24)` | Focused fixed gate: `25 passed in 30.30s`; affected broad group: `339 passed in 1346.10s`; release/preflight: `105 passed, 3 warnings in 60.06s`; RAM safety OK | Full simulator gate: `2175 passed, 2 skipped, 2 xfailed, 7 warnings in 1610.69s` | Not run; no live hardware flashing/audio confirmation requested |
