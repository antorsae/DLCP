...
- [pending] PF.4 `vendor/gpsim-0.32.1-xtc/` retained one release cycle as oracle reference.
  - notes (2026-05-04): split into two phases.  **Phase 1 DONE** (commit 5a56279): deleted the 30 gpsim-only test files (no rust path; auto-skipped under `DLCP_SIM_BACKEND=rust` so removing them doesn't change the gate result) and the 14 gpsim-only operator scripts (`scripts/{gpsim_*,test_full_boot,test_button_inject,simctl,probe_baudcon_mapping,probe_v171_layer2_chain,capture_v171_early_boot_parity,capture_gpsim_ground_truth,run_phase0_blessing,replay_ground_truth,check_ground_truth_capture}.py`).  Authored `scripts/check_gpsim_excision.py` to lock the deletion inventory and report the phase-2 backlog (AST-walker upgrade in 802e932).  Verified: `DLCP_SIM_BACKEND=rust pytest tests/sim -n 16 -q -m "not slow"` -> **586 passed, 32 skipped, 1 xfailed, 0 failed in 23.38 s** (PF.1 stays green).  **Phase 2 deferred** (planning doc landed in commits a049114 -> abf5db6 -> b915d35 -> 57d06e5): the 6 wrapper modules (`src/dlcp_fw/sim/{chain_gpsim,wire_chain_gpsim,control_gpsim,main_gpsim,main_gpsim_timer3,gpsim}.py`), `vendor/gpsim-0.32.1-xtc/`, and `artifacts/tools/gpsim-xtc/` STAY because 41 still-`@pytest.mark.dual_supported` test files retain wrapper imports (mostly `if dlcp_sim_backend == "gpsim":` conditional branches; a small minority have utility-only imports or module-scope wrapper references).  Phase 2 = per-file surgery on those 41 files + the wrapper / vendor / build-artifacts deletion + conftest cleanup.  Estimated 8-17 hours multi-session work; per-file plan in `docs/PF4_PHASE2_PLAN.md`; tracked under the "Phase-2 surgery: 41 dual_supported test files" follow-up entry in the P4 followup tracker.

- **Phase-2 surgery: 41 dual_supported test files retain `from dlcp_fw.sim.{wrapper}` imports** (deferred follow-up from PF.4 phase 1; corrected 2026-05-04 from 40 -> 41 after the codex review of 5a56279 broadened the import-grep to catch `from dlcp_fw.sim import {wrapper}` package re-export shape).  Each of the 41 files needs:
    1. Remove the `from dlcp_fw.sim.{chain,wire_chain,control,main,main_*_timer3,gpsim}_gpsim import ...` lines.
    2. Remove or simplify any `if dlcp_sim_backend == "gpsim":` conditional branches in the test bodies.
    3. Drop any `@pytest.mark.gpsim` decorators (gpsim is gone after phase 2).
  Once all 41 are clean, `scripts/check_gpsim_excision.py` reports 0 phase-2 import sites.  At that point, delete the 6 wrapper modules + `vendor/gpsim-0.32.1-xtc/` + `artifacts/tools/gpsim-xtc/` and the rust silicon-ring engine is the only path left.  The 41-file list is the output of the script's "Phase-2 inventory" report; per-file surgery shape is documented in `docs/PF4_PHASE2_PLAN.md`.
