...
- [in_progress] P4.7 Migrate remaining sim tests (chain, wire, multi-MAIN, etc.)
  ...
  - status: 402 of 800 still-skipped tests unblocked under DLCP_SIM_BACKEND=rust by tagging 36 test files dual_supported (commits 5f67aab + 47411c6 + 4e1cfcd + 77e74f2 + 4e0978a).  398 tests remain skipped; almost all are tied to `chain_gpsim` / `wire_chain_gpsim` / `control_gpsim` runtime harnesses and require actual rust-side adapter migration, not marker batching.
