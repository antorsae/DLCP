...
- [in_progress] P4.5 Migrate `test_v171_*` tests
  ...
  - status: 15 of 16 v171 files migrated to dual-mode (152 of ~176 tests covered + 3 cleanly skipped as gpsim-only breakpoint tests).  Remaining file: `test_v171_v32_layer5_diag_chain.py` (13 wire-chain tests, 4 of which are pre-existing xfailed for a known wire-chain-harness bug per Task #22).  Migration requires per-MAIN `read_main_reg(main_idx, addr)` / `write_main_reg(main_idx, addr, value)` rust facade methods plus multi-MAIN navigation helpers; deferred as a P4.5 follow-up sub-task.
