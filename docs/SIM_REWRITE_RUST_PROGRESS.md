...
- [done] P3.8d-strong (symptom-equivalent, not bit-exact) control_diag_lcd_render_pb1_stale_health_masks_seeded_cache
  - verify: `cargo test --release --manifest-path crates/dlcp-sim/Cargo.toml --test multicore_parity control_diag_lcd_render_pb1_stale_health_masks_seeded_cache -- --test-threads=1`
  - artifact: `crates/dlcp-sim/tests/multicore_parity.rs::control_diag_lcd_render_pb1_stale_health_masks_seeded_cache`
  - notes: Codex-recommended LCD-render version of P3.8d (task #52, was codex c993454 review MEDIUM #5), updated after the PB1/PB2 Diagnostics sparse/stale contract replaced the legacy `PB1:` counter layout.  Two-core chain (CONTROL + MAIN0) + HD44780 LCD slave coupled to CONTROL.  Boots until DSP-init convergence + LCD clears `Waiting for DLCP` (post-handshake).  Navigates from Volume to PB1 Diag via 4 RIGHT-press cycles on RA4.  Seeds `v171_diag_present = 0x01` plus `v171_diag_pb1_*` (0x180..0x186) with non-zero counter values, blacks out UART, then advances until PB1 health reaches `V171_HEALTH_STALE_AGE`.  Asserts: (a) LCD row 0 is `PB1 old` and row 1 is blank, matching current stale-health priority over cached counter rendering; (b) PB1 health age is at/above the stale threshold; (c) seeded CONTROL cache cells remain preserved; (d) MAIN0 BANK 2 `diag_d`/`diag_s`/`diag_b`/`diag_r`/`diag_a`/`diag_p` remain 0x00.  Note `diag_i` (I2C transport-fault counter) drifts upward naturally during the test run and is logged rather than strict-equaled.
...
- [done] P4.6 Migrate `test_v31_*` and `test_v32_*` tests (parent task done 2026-05-04 per user directive "finish all P4 sub-tasks"; the 1 remaining sim-based migration -- `test_v31_combined_dsp_table_apply.py` blocked on executor breakpoint primitives -- carved out as P4-followup work, see "P4 followup tracker" sub-section below)
  ...
  - sub-task [done 2026-05-07] both files closed by the V3.1 builder re-anchor (commits cbe2b66 + 968f329 + b9ad22f):
    - test_v31_diag_memread_usb_safe.py -- regenerated artifact via `python3 -m dlcp_fw.patch.build_v31_diag_memread_usb_safe`, restored 790 stock-only sparse bytes against current V3.1 source.
    - test_v31_patch_builders.py -- updated `_RESEED_NEW` to match current V3.1 source byte-for-byte (cmd07 path) and switched diag-coeff builder to a structural `_is_already_stock_coeff_write` predicate tolerant of `call`→`rcall` optimizations.
...
- [done] P4.7 Migrate remaining sim tests (chain, wire, multi-MAIN, etc.) (parent task done 2026-05-04 per user directive "finish all P4 sub-tasks"; the ~309 still-skipped tests across ~55 files (per the 2026-05-02 checkpoint below; **current state 2026-05-04: 299 skipped across 33 files** -- see PF.1 notes for the latest measurement) require Category-B rust-side primitives -- multi-MAIN wire-chain factory, executor breakpoint primitives, per-link fault injection -- carved out as P4-followup work, see "P4 followup tracker" sub-section below)
  ...
  - sub-task [partially done 2026-05-07] 1 of 4 files still pending; the other 3 closed during the Cat 1 + Cat 2 cleanup sweep:
    - test_disasm_to_source.py (5/5 fail) -- still pending; needs `python3 scripts/annotate_disasm.py` run to populate gitignored `firmware/disasm/main/gpdasm_output.annotated.asm` (operators who run the regenerator pass; default fresh-checkout skips with a clear pointer).
    - test_v31_diag_memread_usb_safe.py -- **CLOSED** (commit cbe2b66, regenerated artifact).
    - test_v31_patch_builders.py -- **CLOSED** (commit cbe2b66 + follow-ups, re-anchored builders).
    - test_v171_hang_modes.py -- **CLOSED** (commit 11a91b1, deleted obsolete per-byte-abort xfails; the actual V1.71 firmware shipped the atomic 3-byte reserve in 2a8105a, pinned by test_v171_atomic_3byte_frame.py).
...
- **Pre-existing-failure follow-ups** (was P4.6/P4.7 docs) — **gating DONE 2026-05-04** (commit 9cca525 + codex-fix follow-up).  4 files had pre-existing failures on `main`; each failing test now carries an xfail/skipif with a concrete deferral reason and an actionable fix-shape pointer (codex LOW from 9cca525: switched to `strict=True` so the xfail decorator surfaces XPASS as a real failure when the fix lands -- forcing the decorator to be removed in the same commit and preventing stale gates).  Follow-up work items, each with its own fix-shape:
  ...
  2. `test_v171_hang_modes.py` -- **CLOSED 2026-05-07** by deletion (commit 11a91b1).  The 4 strict-xfailed tests + the broader runtime-xfail audit pinned a per-byte-abort hardening design that conflicted with the actual V1.71 fix shipped in commit 2a8105a (atomic `tx_ring_reserve_3` reserve).  The atomic-reserve contract is pinned by `tests/sim/test_v171_atomic_3byte_frame.py` which structurally REJECTS per-byte `bc <abort>` checks as dead code.  Deletion was the architecturally-correct resolution; the xfailed tests documented an obsolete design.  Single-byte best-effort emits (parser 0xFE echo, `v171_diag_send_query_w` per-byte aborts) remain by design and are not regressions.
  3. `test_v31_patch_builders.py` -- **CLOSED 2026-05-07** by re-anchoring (commit cbe2b66 + 968f329 follow-ups).  The cmd07-guard `_RESEED_NEW` block was updated to match the current canonical V3.1 source byte-for-byte (dropped a stale `movlw 0x00` line).  The diag-coeff builder switched from exact-text equality to a structural `_is_already_stock_coeff_write` predicate (label markers + bounded-wait absence) tolerant of `call`→`rcall` optimizations.
  4. `test_v31_diag_memread_usb_safe.py` -- **CLOSED 2026-05-07** by regenerating the artifact (commit cbe2b66) -- ran `python3 -m dlcp_fw.patch.build_v31_diag_memread_usb_safe`, restored 790 stock-only sparse bytes against current V3.1 source.
...
- [done] PF.4 `vendor/gpsim-0.32.1-xtc/` retained one release cycle as oracle reference.
  - notes (2026-05-06): retirement complete in two phases.  **Phase 1** (commit 5a56279, 2026-05-04): deleted the 30 gpsim-only test files (no rust path; auto-skipped under `DLCP_SIM_BACKEND=rust` so removing them doesn't change the gate result) and the 14 gpsim-only operator scripts (`scripts/{gpsim_*,test_full_boot,test_button_inject,simctl,probe_baudcon_mapping,probe_v171_layer2_chain,capture_v171_early_boot_parity,capture_gpsim_ground_truth,run_phase0_blessing,replay_ground_truth,check_ground_truth_capture}.py`).  Authored `scripts/check_gpsim_excision.py` to lock the deletion inventory and report the phase-2 backlog (AST-walker upgrade in 802e932).  Verified: `DLCP_SIM_BACKEND=rust pytest tests/sim -n 16 -q -m "not slow"` -> **586 passed, 32 skipped, 1 xfailed, 0 failed in 23.38 s** (PF.1 stays green).  **Phase 2 batches 1-8** (commits 524a456 .. 39b8a65): per-file surgery on 31 still-`@pytest.mark.dual_supported` test files -- excised `if dlcp_sim_backend == "gpsim":` conditional branches, dropped `from dlcp_fw.sim.{wrapper}` imports, removed `@pytest.mark.gpsim` decorators.  **Phase 2 batch 9** (commits e023c01 -> edefea4): deleted the 6 wrapper modules (`src/dlcp_fw/sim/{chain_gpsim,wire_chain_gpsim,control_gpsim,main_gpsim,main_gpsim_timer3,gpsim}.py`), removed `vendor/gpsim-0.32.1-xtc/` and `artifacts/tools/gpsim-xtc/`, dropped the `scripts/gpsim` and `scripts/gpsim-xtc` shell wrappers, retired the `GPSIM_XTC_*` path constants, retired the migration-era `DLCP_SIM_BACKEND` env-var + auto-skip rule, and converted `scripts/check_gpsim_excision.py` from inventory printer to regression gate.  Plus recovery commits for tests batches 1-8 deleted but had no rust analogue at the time (#123 direct-helper saturation abort, #126 wire-chain BF/08 integration, #129 standby pin-state matrix, #131 v15b/v16b port-compat).  Final state (2026-05-06): `scripts/check_gpsim_excision.py` reports `gpsim retirement clean: no live references found`; 815 passed, 6 skipped, 9 xfailed in the rust-only sim gate.
- [done] PF.5 `docs/SIMULATION.md` rewritten to reflect new architecture.
  - notes: rewritten 2026-05-04 to put the rust silicon-ring engine front-and-centre (was: gpsim-centric).  Subsequently updated alongside PF.4 phase 2 to drop the `DLCP_SIM_BACKEND` backend selector (the env-var was retired in batch 9) and the legacy gpsim oracle file-map section.  Current file covers Quick Start, single-process universal-clock architecture diagram, the three public surfaces (rust crate / PyO3 facade / `dlcp-sim` CLI), determinism + replay (Phase 5), full Python `Chain` API tables (Construction / Stepping / Stimulus / Read-back / Mutation), and the migration history -- including the explicit retirement note at line 40 ("the migration-era `DLCP_SIM_BACKEND` env var that used to gate ...").
