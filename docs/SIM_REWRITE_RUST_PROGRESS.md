...
- [completed] P3.6a Multicore parity — architectural retirement of Task #22 bridge-mirror echo-loop
  - verify: `cargo test --release --manifest-path crates/dlcp-sim/Cargo.toml --lib three_core_silicon_ring_uart_topology_has_no_echo_or_duplicates && cargo test --release --manifest-path crates/dlcp-sim/Cargo.toml --test multicore_parity three_core_ring_v171_v32_v32_boots_under_silicon_topology three_core_synthetic_pb2_injection_decrement_and_forward -- --ignored`
...
- [pending] P3.6b CONTROL BF/2N diag reply processing parity — **blocked on hardware data**
  ...
  - notes: With proper firmware-driven menu navigation (4 RIGHT-press cycles drive state 0 -> 4 = PB1 Diag), CONTROL enters `v171_diag_pb_screen`, fires `cmd 0x22` + `cmd 0x21` queries (CTL.TX shows `B1/22/00` + `B1/21/00`), MAIN0 emits the 7-frame `BF/21..BF/27` reply burst, and MAIN1 forwards the full burst to CONTROL.RX (verified by direct byte-window inspection `MAIN1.TX[147..150] = BF 24 00 BF 25 00 BF 26 00 BF 27 00`).  But CONTROL's parser **never successfully processes** BF/27 through the BF/2N last-frame path: `v171_diag_target` trajectory `[00]` (never toggled), `v171_diag_present = 0x00` (never set), `v171_diag_flags = 0x06` (PENDING bits never cleared).  Same symptom gpsim Python harness shows; both simulators agree.  No real-hardware data confirms PB2 reply convergence -- the existing `tests/hardware/test_live_state_transitions.py:211+` only validates LCD render, not data landing.  Per codex strategic review (this branch), this task is therefore re-scoped: it cannot be resolved by sim work alone until hardware confirms whether PB2 reply convergence is a real V1.71 feature or a latent firmware bug.  **Path 1 to unblock**: extend the hardware Layer 5 test to capture LCD content from PB2 Diag screen (state 5) and assert PB2's per-counter cells render real values (digits / A-E / +) rather than placeholder.  If hardware also stalls at PB1, V1.71 is suspect.  If hardware lands PB2 cleanly, both gpsim and Rust have a shared timing/electrical fidelity gap to chase.  Open hypotheses (in the probe docstring): parser frame-state stuck mid-frame, dispatch chain shadows cmd 0x27, RXIF ISR drops bytes, transient OERR during the BF burst.
