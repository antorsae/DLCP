...
- [in_progress] P4.6 Migrate `test_v31_*` and `test_v32_*` tests
  ...
  - sub-task [pending] 4 sim-based files NOT migrated, each with discrete blockers warranting dedicated sub-tasks:
    ...
    - test_v31_review_findings.py (8 tests) -- BLOCKER: needs both (a) I²C fault injection on the rust TAS3108 slave model (`harness.set_i2c_fault("dsp34", address_nack_count=N)` / `clear_i2c_faults()`) AND (b) MSSP STOP-bit fault injection on the rust 2455 MSSP peripheral (`harness.set_mssp_stop_fault(...)` / `clear_mssp_stop_faults()` -- used by `test_idle_wait_blocks_during_pen_fault_then_recovers`, `test_dsp_path_degraded_during_mssp_stop_fault`, and `test_pen_timeout_firmware_detects_before_sspcon2_poke` at tests/sim/test_v31_review_findings.py:211, :273, :417).  Adding `set_i2c_fault` to `crates/dlcp-sim/src/peripherals/tas3108.rs` + `set_mssp_stop_fault` to `crates/dlcp-sim/src/peripherals/mssp.rs` + plumbing through the Chain pyclass is a discrete sub-task.
