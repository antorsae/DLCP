...
Status: archived historical V1.71/V3.2 ledger.  These rows were active during
the former V1.71/V3.2 release-candidate period; current release work is tracked
in the V3.4/V1.73 docs and field bug ledger.

This ledger tracks product-firmware bugs in the former V1.71/V3.2
CONTROL/MAIN candidate pair.
It is separate from:
...
| BUG-IR-01 | green | CONTROL IR | Earlier real-IR gates did not move MAIN state, but the operator later explicitly retested real IR and reported that it works on both stock V1.6b and V1.71. V1.71 uses the stock V1.6b in-ISR RC5 decoder path; the failed Timer1/deferred receiver bodies are deleted so they cannot be accidentally re-wired. | `tests/sim/test_v171_ir_rc5_pulse_train.py::test_v16b_and_v171_rc5_pulse_train_decode_same_command_stress`, `tests/sim/test_v171_hang_modes.py::test_v171_ir_decode_uses_hardware_validated_stock_isr_path`, and `test_v171_profile_ir_actions_match_stock_v16b_dispatch_behavior`. |
...
- `test_v171_ir_decode_uses_hardware_validated_stock_isr_path` pins the
  current firmware shape: V1.71 uses the stock-compatible in-ISR RC5 decoder
  path and the failed Timer1/deferred receiver symbols stay deleted.
