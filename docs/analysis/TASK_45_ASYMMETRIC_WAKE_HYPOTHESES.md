...
**Status:** H1 firmware-path reachability proven in rust sim (no MCLR-hold, no UART drop) — the test models the H1 *claim* (MAIN1's AN0 is held below threshold while MAIN0's wake completes) and shows the firmware then stays inside `adc_boot_gate` indefinitely. This is evidence for H1 *reachability* (the firmware can land in this state from rail dynamics alone), NOT independent evidence that the real PCB has the asserted asymmetric coupling — that still hinges on the open question in `docs/analysis/PIN_SEMANTICS.md:105-107` (RA6/RB3/RB4 → connector continuity unproven) and on the hardware scope traces enumerated in §B1. 3 ranked hypotheses (codex investigations 2026-05-02 and 2026-05-02-pass-2, codex review of 9960ebe 2026-05-02-pass-3) with rust-sim reproduction paths and HW confirm/deny tests for each.
...
**A4. Build the H1 rail-wait reproduction via RailCoupler model (DONE 2026-05-02, refined post codex review of 9960ebe)**: a 3-core test (`v171_v32_v32_asymmetric_wake_railcoupler_spontaneous_no_fault`) that lets both MAINs process the WAKE byte normally (AN0 stays at the 0x0300 build seed during STDBY/WAKE injection), then watches MAIN1's `diag_b` byte (RAM 0x2E8). The first chunk where `diag_b1` transitions 0 → 1 proves MAIN1 has executed `standby_event_dispatch` (asm:8392) and is now inside (or about to enter) `adc_boot_gate` via the `call adc_boot_gate` at asm:8393. The test sets MAIN1's AN0 to 0x0100 at that moment (the 10 ms `timer3_blocking_delay_ms_W` at asm:4051 inside `adc_boot_gate_loop` provides ~360 chunks of slack, so the droop is in place before MAIN1's first sample-read at asm:4054). MAIN0's AN0 is left at 0x0300 throughout — modelling the H1 *claim* that MAIN0's local sense node is closer to the regulator and so does NOT droop when MAIN0's amps engage and load the shared rail. Result: MAIN1 ends at `active_flags.bit3 = 1` (from `wake_request_handler`, asm:1894), `diag_b >= 1` (from `standby_event_dispatch`), and the post-gate latch raises (`LATB.bit3`/`LATB.bit4` at asm:4084/4101, `LATA.bit6` at asm:4098) NEVER fire — i.e. MAIN1 IS in the `adc_boot_gate` rail-wait. A companion baseline (`v171_v32_v32_wake_baseline_main1_progresses_without_fault`) confirms that with no AN0 manipulation MAIN1 reaches `db=1` and the post-gate latch raises, so the stuck-state in the RailCoupler test is attributable to the AN0 droop and not to a generic chain-forwarding sim gap. NO MCLR-hold, NO UART drop. SRC4382 GPO writes were NOT required to land the asymmetric outcome; the early amp-enable transitions (RB4 at asm:4084, RA6 at asm:4098, RB3 at asm:4101) are sufficient diagnostic signal. The test does NOT prove the real PCB has the asserted asymmetric coupling — that still hinges on the netlist continuity question in `docs/analysis/PIN_SEMANTICS.md:105-107` and the §B1 hardware scope traces.
