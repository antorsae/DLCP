...
**A4. Build the H1 firmware-observable reproduction via RailCoupler model (DONE 2026-05-02, refined post codex reviews of 9960ebe and acd45fb)**: a 3-core test (`v171_v32_v32_asymmetric_wake_railcoupler_spontaneous_no_fault`) that lets both MAINs process the WAKE byte normally (AN0 stays at the 0x0300 build seed during STDBY/WAKE injection), then watches MAIN1's `active_flags.bit3` (RAM 0x05E). The first chunk where `af1.bit3` transitions 0 → 1 proves MAIN1 has just executed `wake_request_handler` (asm:1894) and is somewhere between the parser exit and the gate's first ADC conversion at asm:4048. The test drops MAIN1's AN0 to 0x0100 on that chunk; the path from `wake_request_handler` exit to the first conversion latch traverses several main-loop service routines + `standby_event_dispatch` (asm:8386) + the gate prologue, totalling many MAIN-Tcy, so the droop lands well before the latch. MAIN0's AN0 is left at 0x0300 throughout — modelling the H1 *claim* that MAIN0's local sense node is closer to the regulator. Result: MAIN1 ends at `active_flags.bit3 = 1`, `diag_b >= 1`, and the post-gate amp-enable latches (`LATB.bit3`/`LATB.bit4`/`LATA.bit6` at asm:4101/4084/4098) NEVER raise — the externally-visible "RIGHT MAIN's amp didn't turn on after wake" symptom from the field report. A companion baseline (`v171_v32_v32_wake_baseline_main1_progresses_without_fault`) confirms that without the droop MAIN1 fully wakes, so the stuck amp-pin state in the RailCoupler test is attributable to the AN0 droop and not to a generic chain-forwarding sim gap. NO MCLR-hold, NO UART drop. The test does NOT pin MAIN1's PC to `[adc_boot_gate, adc_boot_gate_exit)` strictly — codex's pass-3+4 reviews surfaced that chunked-simulator granularity, ADC conversion latch timing, and downstream wake-init dependencies (I2C writes / DSP coeff bursts that can themselves stall under low AN0) collectively prevent a bit-precise PC claim. The current test even observes some MAIN1 TX activity post-droop (consistent with the parser forwarding chain heartbeat frames from CONTROL through the ring). What the test asserts is the firmware-OBSERVABLE form of H1: MAIN1 dispatches wake but its amp pins never come up. The test does NOT prove the real PCB has the asserted asymmetric coupling — that still hinges on the netlist continuity question in `docs/analysis/PIN_SEMANTICS.md:105-107` and the §B1 hardware scope traces.
