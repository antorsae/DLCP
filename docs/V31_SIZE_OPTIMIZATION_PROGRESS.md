...
- `high-risk / do not touch`:
  `hid_command_dispatch`, `fw_update_relay`, `cmd_dispatch_gated`, `uart_link_parser_drain_rx_and_forward`, `i2c_apply_channel_route_sync_burst`, `poll_src4382_route_monitor`, `run_wake_rail_gate_and_dsp_cold_init`, `flash_write (V3.1: preset B address remap prologue)`, `usb_sie_endpoint_pump`, `isr_high_priority_dispatch`, `fw_update_signature_status_word_helper`, `flash_erase (V3.1: preset B address remap prologue)`, `i2c_byte_tx (V3.1 enhanced)`, `adc_divide_staged_words`, `i2c_tas3108_reg1f_write (V3.1 enhanced)`, `i2c_tas3108_coeff_write (V3.1 enhanced — Fix F: boot-gated PEN)`, `i2c_secondary_dev_write (V3.1 enhanced)`, `i2c_wait_bus_idle (stock — unbounded spin)`, `hard_reset`
...
- `optimize locally`:
  `shift_003_006_right_clear_c`, `i2c_receive_sspbuf_bounded`, `rx_ring_has_data`, `uart_tx_byte_blocking (V3.1 enhanced)`, `send_status_burst`, `report_cmd29_status`, `fw_update_emit_bf18_status`
- `share/reuse candidate`:
  `send_status_burst`, `uart_emit_formfeed_colon_text_line`, `report_cmd29_status`, `fw_update_emit_bf18_status`
- `resolved dead code / removed in W02-R01`:
  `wait_mssp_idle_bounded`, `recover_mssp`
- `keep`:
  `hid_out_payload_index_to_fsr2`, `hid_config_fill_ff_at_index`, `nibble_to_hex_ascii`, `restore_eeprom_settings_on_boot`, `stage_hid_ep1_in_report_from_selector`, `copy_indexed_fsr2_byte_to_hid_ep1_in`, `float32_add_secondary_to_primary_in_place`, `twos_complement_024_027_after_low_byte_complement`, `main_core_service_2650`, `persist_dirty_runtime_state_to_eeprom`, `float32_exp_limit1024_in_place`, `float32_multiply_primary_by_secondary_in_place`, `add_shifted_multiplicand_to_product_accumulator`, `shift_multiplier_mantissa_right_clear_c`, `main_core_service_2bac`, `fw_update_commit_hid_payload_page`, `float32_divide_primary_by_secondary_in_place`, `main_core_service_2d80`, `float32_to_int32_in_place`, `main_core_service_30cc`, `float32_pack_mantissa_exponent_sign`, `i2c_secondary_apply_wake_init_table`, `truncate_float32_to_integral_float_in_place`, `usb_ep0_apply_clear_set_feature_request`, `format_uint16_radix_ascii_to_w_pointer`, `boot_init_peripherals_and_enter_adc_gate`, `usb_ep0_stage_in_data_packet`, `usb_ep0_prepare_in_data_copy_pointers`, `usb_ep0_store_in_data_byte_and_advance`, `usb_ep0_dispatch_standard_setup_request`, `usb_ep0_prepare_get_status_reply`, `usb_ep0_select_get_descriptor_payload`, `read_low_memory_byte_at_tblptr`, `preset_table_apply_entry_legacy_blocking`, `main_core_service_38a2`, `adaptive_baud_select`, `i2c_emit_tas3108_coeff_from_staged_float`, `usb_hid_dispatch_out_report_if_ready`, `uart_rx_with_framing`, `hw_standby_shutdown`, `usb_ep1_out_copy_packet_if_ready`, `int32_to_float32_and_save`, `float32_multiply_ram_window_by_staged_operand_in_place`, `float32_add_staged_operand_to_ram_window_in_place`, `intel_hex_checksum_update`, `usb_ep1_in_copy_scratch_buffer_to_bdt`, `usb_ep0_arm_out_pingpong_bd`, `usb_bus_reset_reinitialize`, `format_int16_decimal_ascii_to_w_pointer`, `usb_apply_set_configuration`, `i2c_secondary_dev_random_read`, `adc_remainder_staged_words`, `flash_write_with_gie_off`, `usb_ep0_service_setup_transaction`, `main_core_service_432e`, `uart_tx_ascii_hex_byte`, `hex_scratch_nibble_to_ascii`, `eeprom_write_blocking`, `nvm_unlock_and_set_wr`, `usb_ep0_service_in_transaction`, `map_audio_source_selector_to_route_pair`, `timer3_blocking_delay`, `uart_emit_formfeed_colon_text_line`, `drive_audio_route_select_latches`, `uart_config`, `preset_replay_selected_table_blocking`, `usb_hid_mailbox_send_reply_if_ready`, `uint8_to_float32_and_save`, `rx_ring_read`, `usb_ep1_configure_hid_buffers`, `fw_update_emit_zero_status_lines`, `uart_tx_block_from_buffer`, `uart_tx_block_load_indexed_byte`, `eeprom_write_byte_if_changed`, `main_usb_service_4700`, `main_usb_service_4720`, `clear_ram_span_from_staged_addr_count`, `usb_poll_host_presence_reinit_or_shutdown`, `timer3_arm_interrupt_countdown`, `standby_event_dispatch`, `mssp_hard_reset`, `run_main_service_pass`, `usb_delay_countdown_with_clrwdt`, `usb_disconnect_wait_clear_state`, `usb_clear_activity_interrupt_after_settle`, `uart_rx_ring_drain_all`, `eeprom_read_byte`, `timer0_rearm_50ms_heartbeat`, `i2c_tas3108_reg1f_02_clear_source_pins`, `usb_shutdown`, `usb_ep1_configure_if_enabled`, `timer3_timeout_elapsed_carry`, `usb_activity_settle_delay_with_clrwdt`, `timer3_blocking_delay_1ms`, `uart_reconfigure_and_resync_parser`, `timer3_blocking_delay_2ms`, `timer3_stop_interrupt_countdown`, `main_core_service_4954`, `main_uart_service_495e`, `usb_disconnect_handler`, `main_i2c_service_4966`, `main_core_service_496c`
...
| W02 | W02-E08 | `Ptolemy` | `usb_ep0_service_setup_transaction`, `main_core_service_495a` | Replace single-use wrapper call with direct call to `usb_ep0_dispatch_hid_setup_request` and delete wrapper | yes | 15145 | `0x4B8B` | 116 | -41 | `35 passed` fast checks + `78 passed, 2 skipped` | parent accepted into `W02-R01` | yes | low | Shared-workspace run. Wrapper had one callsite and only a `goto` body. Parent delta is advisory only. |
...
| W03 | W03-E01 | `Bernoulli` | `shift_003_006_right_clear_c`, `main_core_service_496c` | Remove empty return wrapper and its single remaining callsite if the net size drops | in progress | n/a | n/a | n/a | n/a | not run yet | launched | no | low | Isolated worktree: `artifacts/reanalysis/size_opt_w03/W03-E01` |
| W03 | W03-E02 | `Popper` | `adaptive_baud_select`, `uart_reconfigure_and_resync_parser` | Inline the single-call UART init wrapper if the net size drops | in progress | n/a | n/a | n/a | n/a | not run yet | launched | no | low | Isolated worktree: `artifacts/reanalysis/size_opt_w03/W03-E02` |
| W03 | W03-E03 | `Poincare` | `usb_clear_activity_interrupt_after_settle`, `usb_activity_settle_delay_with_clrwdt` | Inline the single-call USB timer wrapper via direct inner-label call if it assembles cleanly and saves bytes | in progress | n/a | n/a | n/a | n/a | not run yet | launched | no | medium | Isolated worktree: `artifacts/reanalysis/size_opt_w03/W03-E03` |
...
| W03 | W03-E06 | `Newton` | single `timer3_stop_interrupt_countdown` callsite plus wrapper | Inline the timer-stop wrapper after `uart_rx_with_framing` if the net size drops | in progress | n/a | n/a | n/a | n/a | not run yet | launched | no | medium | Isolated worktree: `artifacts/reanalysis/size_opt_w03/W03-E06` |
...
| W03 | W03-E08 | `Parfit` | `run_wake_rail_gate_and_dsp_cold_init` callsite plus `main_i2c_service_4966` | Inline the single-call MSSP disable wrapper if the net size drops | in progress | n/a | n/a | n/a | n/a | not run yet | launched | no | high | Isolated worktree: `artifacts/reanalysis/size_opt_w03/W03-E08` |
...
| W07 | W07-E01 | `coordinator` | `cmd_dispatch_gated` array mapping and hoisting | Eliminate redundant `0x5F` mapping into `ram_0x0F1..0x0FC` before `preset_table_apply_entry_legacy_blocking` across 6 branches | yes | 14783 | `0x4A21` | 478 | -87 | smoke `31 passed, 1 xfailed` (`2169.78s`) | parent accepted into `W07-R01` | yes | medium | Replaced the dead `0x0F` staging array with direct `ram_0x013/0x014` loads and hoisted the common `0x5F` high byte once for the whole six-call block. |
| W07 | W07-E02 | `coordinator` | `cmd_dispatch_gated` repetitive I2C writes | Create local helper to collapse 4 instances of repetitive `i2c_secondary_dev_write` sequences around `cmd_dispatch_gated__route_code_1_i2c_pair` | yes | 14831 | `0x4A51` | 430 | -39 | smoke `31 passed, 1 xfailed` (`2175.93s`) | parent accepted into `W07-R01` | yes | low | Added a local `cmd_dispatch_gated_i2c_pair` helper that preserves the original `0x0D` then `0x08` write order and reuses `ram_0x00D` as scratch for the second payload byte. |
| W07 | W07-E03 | `coordinator` | `send_status_burst` postamble helper | Extract sequential `uart_tx_byte_blocking` and `timer3_blocking_delay_1ms` calls into a shared `rcall` postamble helper | yes | 14831 | `0x4A69` | 406 | -39 | smoke `31 passed, 1 xfailed` (`2176.39s`) | parent accepted into `W07-R01` | yes | low | Added `send_status_burst_postamble` for the first four delayed status bytes and kept the final undelayed `BF/1D` tail untouched. |
| W07 | W07-E04 | `coordinator` | Single-call wrapper inlining (Part 1) | Inline `uart_reconfigure_and_resync_parser`, `main_core_service_496c`, `main_i2c_service_4966`, and `timer3_stop_interrupt_countdown` | yes | 14848 | `0x4A63` | 412 | -22 | smoke `1 failed, 30 passed, 1 xfailed` (`2214.73s`) | reject / regression | no | medium | Regressed `tests/sim/test_reconnect_wake_gate.py::test_v31_v163b_wire_chain_standby_reconnect_dsp_gate`. The helper-inline cluster was dropped intact rather than trying to salvage individual edits from the regressing batch. |
| W07 | W07-E05 | `coordinator` | Single-call wrapper inlining (Part 2) | Inline `usb_disconnect_handler`, `usb_activity_settle_delay_with_clrwdt`, and `main_core_service_4954` | yes | 14856 | `0x4A6B` | 404 | -14 | not run | reject / low-gain holdback | no | medium | Assembled cleanly, but the 14-byte gain was not worth changing the only-site `CLRWDT` topology and helper jump/return structure once `W07-R01` already met the wave target without it. |
...
| W08 | W08-E01 | `coordinator` | `float32_exp_limit1024_in_place` counted-loop rewrite | Replace the 10x unrolled `movff ram_0x02F..032 -> ram_0x025..028 ; movlw 0x2F ; call float32_multiply_ram_window_by_staged_operand_in_place` cluster with a semantically exact counted loop | yes | 14529 | `0x4923` | 732 | -190 | `21 passed` (`1247.70s`) | parent accepted into `W08-R01` | yes | high | Reused `ram_0x011` as the loop counter only after auditing `float32_multiply_ram_window_by_staged_operand_in_place` and its callees to confirm they never clobber it. |
...
| W08 | W08-E04 | `coordinator` | Final redundant `iorlw 0x00` removal | Remove the lone remaining `iorlw 0x00` in `hid_diag_memread_dispatch` after `movf ram_0x11E, W` already sets `STATUS.Z` | yes | 14717 | `0x49DF` | 544 | -2 | canonical checks `5 passed, 2 skipped` (`158.60s`) | parent accepted into `W08-R01` | yes | low | The only red follow-up was non-gate `test_diag_memread_usb_safe_hex_only_restores_stock_sparse_gaps`, which reflects the derived USB-safe HEX not being rebuilt from the new canonical release. |
...
- `W02-R01` accepted actions:
  ...
  - `i2c_receive_sspbuf_bounded`: collapsed duplicated `SSPCON1 & 0x0F` extraction/compare logic
  ...
  - `main_core_service_495a`: removed single-use `goto` wrapper via direct call to `usb_ep0_dispatch_hid_setup_request`
...
- `W07-R01` accepted actions:
  ...
  - collapsed the four repeated `cmd_dispatch_gated` I2C write pairs
    ...
    the original write order and `i2c_tas3108_reg1f_02_clear_source_pins` tail
...
- `W08-R01` accepted actions:
  - rewrote the 10x repeated `float32_exp_limit1024_in_place` copy/apply cluster
...
- 2026-04-11: external review of the `W01-E01` rejection found its "no binary delta" result inconsistent with `.lst` evidence — each `movff X, X` self-move emits 4 bytes of object code, and `assemble_v30()` does no padding injection. Reopened self-move removal as `W04-E03..E05` with mandatory content-diff validation per the new `## Measurement Gotchas` section in the spec. First target: the cluster at `dlcp_main_v31.asm:2691-2694` inside `float32_add_secondary_to_primary_in_place`, which is the no-op branch of a conditional copy paired with a sibling branch at 2673-2676.
...
- 2026-04-12: queued `W08` against the accepted `W07-R01` baseline.
  ...
  `float32_exp_limit1024_in_place` counted-loop rewrite plus several smaller
