...
- Reproduce `test_v171_v32_layer5_diag_chain.py::test_v171_v32_layer5_chain_diag_page_polls_pb1_and_pb2` (the Task #22 XFAIL).  **Status (2026-04-27): split into P3.6a / P3.6b** — see `SIM_REWRITE_RUST_PROGRESS.md`.  P3.6a (architectural retirement of the bridge-mirror echo loop) is **completed** — the Rust silicon-correct ring is structurally incapable of producing the gpsim PTY-bridge mirror echo, proven by synthetic + firmware-driven probes.  P3.6b (CONTROL BF/2N reply-path processing parity, i.e. `v171_diag_present` reaching `0x03`) is **open**.  Hardware probe ran on real DLCP rig 2026-04-27; LCD camera captures of PB1 Diag (`PB1: I+ D1 SE B4 / RA A3 O8 V6 W+..`) and PB2 Diag (`PB2: I+ D  S+ B / R+ A9  P  OB W9..`) both show the V1.71 Tier-1 Overflow layout (`PBn:` colon prefix → `v171_diag_present.bit_n = 1`), proving **both BF/2N reply convergences work on real silicon**.  V1.71 firmware is correct; gpsim shows PB1-only saturation (PB1 reply lands, PB2 reply lost — Task #22 timing/electrical hypothesis stack); rust shows ZERO replies (task #94, 2026-05-04 empirical: query path never lands either PB1 or PB2).  The two simulator gaps are DISTINCT (gpsim = PB2-only, rust = both-missing) -- the 2026-04-27 working assumption that they were the same shared gap was based on xfailed tests that use `run=False` and never executed empirically on rust.  The four `_V171_V32_PB2_BRIDGE_XFAIL` markers in the Python test stay in place until both gaps close.
...
| Task #22          | Pre-existing tracking task for the `chain_gpsim.py` two-MAIN echo-loop. **Status: split (2026-04-27, refined 2026-05-04)** -- the architectural bridge-mirror echo (P3.6a) is retired by the Rust silicon ring (proven by synthetic + firmware-driven probes); P3.6b (CONTROL `v171_diag_present` saturation) is open as TWO DISTINCT gaps. Real silicon converges cleanly (LCD camera shows Overflow layouts on both PB1 and PB2 pages, `PBn:` colon prefix → `v171_diag_present.bit_n = 1`); V1.71 firmware is therefore correct. gpsim Python harness saturates at PB1 only (PB2 reply lost — Task #22 timing/electrical hypothesis stack). Rust 2026-05-04 empirical (task #94): yields ZERO replies — the Diag-page cmd 0x21 query/reply path never fires for either PB. The 2026-04-27 prediction that rust would reproduce gpsim's PB1-only saturation was assumption-based (the xfailed tests use `run=False` and never executed empirically). The four `_V171_V32_PB2_BRIDGE_XFAIL` markers stay in place until both Task #22 (gpsim) and task #94 (rust) close. |
