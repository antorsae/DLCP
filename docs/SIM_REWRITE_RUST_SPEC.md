...
- Reproduce `test_v171_v32_layer5_diag_chain.py::test_v171_v32_layer5_chain_diag_page_polls_pb1_and_pb2` (the Task #22 XFAIL).  **Status (2026-04-27): split into P3.6a / P3.6b** — see `SIM_REWRITE_RUST_PROGRESS.md`.  P3.6a (architectural retirement of the bridge-mirror echo loop) is **completed** — the Rust silicon-correct ring is structurally incapable of producing the gpsim PTY-bridge mirror echo, proven by synthetic + firmware-driven probes.  P3.6b (CONTROL BF/2N reply-path processing parity, i.e. `v171_diag_present` reaching `0x03`) is a **V1.71 firmware-design feature, NOT a sim fidelity bug** (confirmed via operator HW retest 2026-05-04 with V3.2 rev 0x3F + V1.71 rev 0x0F).  Real HW also shows "PB1/PB2 n/a" after just 4 RIGHT presses; both rust and HW need multiple LEFT/RIGHT navigation cycles to converge to present=0x03.  Probe v21 confirms rust matches: 7 cycles of mixed LEFT/RIGHT yields present=0x03 with PB1+PB2 caches populated.  The 2026-04-27 hardware-probe write-up "shows diag values without further input" was incorrect — the operator must have performed implicit navigation while testing.  Task #94 closed 2026-05-04.  The four `_V171_V32_PB2_BRIDGE_XFAIL` markers stay in place because the no-user-events test gate is non-converging by V1.71 firmware design.  Separate divergence candidate uncovered during the same HW retest: pressing STBY while on a PB1/PB2 Diag page on real HW only dims the CONTROL LCD ("Zzz..." dimmed) but MAINs keep playing music (CONTROL apparently does not broadcast the panel `B0/03/00` STDBY frame from this state).  Probe v23 (2026-05-04, see `docs/analysis/V171_STBY_FROM_DIAG_PROBE_2026-05-04.md`) measured rust at the CONTROL.TX byte-stream level: rust DOES emit `B0/03/*` from PB1 Diag (8 frames, identical pattern to STBY-from-Volume positive control) -- and the V1.71 asm has no menu-state guard in the STBY-edge handler at PC 0x0DB0, so rust is firmware-faithful.  The HW divergence is therefore unresolved without scope-on-wire data; reconciling hypotheses listed in the probe v23 analysis.  Filed as task #95.
...
| Task #22          | Pre-existing tracking task for the `chain_gpsim.py` two-MAIN echo-loop. **Status: split (2026-04-27); P3.6b CLOSED 2026-05-04 after operator HW retest** -- the architectural bridge-mirror echo (P3.6a) is retired by the Rust silicon ring (proven by synthetic + firmware-driven probes); P3.6b (CONTROL `v171_diag_present` saturation) is a V1.71 firmware-design feature, NOT a sim fidelity bug. Real silicon converges cleanly when navigated (LCD camera shows Overflow layouts after multiple navigation cycles); V1.71 firmware is correct. Operator HW retest 2026-05-04 with V3.2 rev 0x3F + V1.71 rev 0x0F confirmed real HW also shows "PB1/PB2 n/a" after just 4 RIGHT presses -- BOTH rust and HW need multiple LEFT/RIGHT navigation cycles to converge to present=0x03 (probe v21: rust converges in 7 cycles).  The 2026-04-27 hardware-probe write-up "shows diag values without further input" was incorrect; the operator must have performed implicit navigation. Task #94 closed (rust matches HW). The four `_V171_V32_PB2_BRIDGE_XFAIL` markers stay in place because the no-user-events test gate is non-converging by V1.71 firmware design. Separate divergence candidate uncovered during same retest: pressing STBY while on a Diag page on real HW only dims the CONTROL LCD ("Zzz..." dimmed) but MAINs keep playing music (CONTROL apparently does not broadcast `B0/03/00` from this state).  Probe v23 (2026-05-04, see `docs/analysis/V171_STBY_FROM_DIAG_PROBE_2026-05-04.md`) measured rust at the CTL.tx byte-stream level: rust DOES emit 8 `B0/03/*` frames, byte-stream-equivalent to the STBY-from-Volume positive control.  The V1.71 asm STBY-edge handler at PC 0x0DB0 has no menu-state guard, so rust is firmware-faithful to V1.71.  HW divergence remains unresolved without scope-on-wire data; reconciling hypotheses listed in the probe v23 analysis (asm-vs-silicon gate not yet identified, MAIN-side gate, or operator-interpretation transient).  Filed as task #95. |
