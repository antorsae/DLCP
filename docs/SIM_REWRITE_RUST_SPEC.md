...
- Reproduce `test_v171_v32_layer5_diag_chain.py::test_v171_v32_layer5_chain_diag_page_polls_pb1_and_pb2` (the Task #22 XFAIL).  **Status (2026-04-27): split into P3.6a / P3.6b** — see `SIM_REWRITE_RUST_PROGRESS.md`.  P3.6a (architectural retirement of the bridge-mirror echo loop) is **completed** — the Rust silicon-correct ring is structurally incapable of producing the gpsim PTY-bridge mirror echo, proven by synthetic + firmware-driven probes.  P3.6b (CONTROL BF/2N reply-path processing parity, i.e. `v171_diag_present` reaching `0x03`) is **open as a test-scenario artifact (NOT a sim fidelity bug)**.  Hardware probe ran on real DLCP rig 2026-04-27; LCD camera captures of PB1 Diag (`PB1: I+ D1 SE B4 / RA A3 O8 V6 W+..`) and PB2 Diag (`PB2: I+ D  S+ B / R+ A9  P  OB W9..`) both show the V1.71 Tier-1 Overflow layout (`PBn:` colon prefix → `v171_diag_present.bit_n = 1`), proving **both BF/2N reply convergences work on real silicon**.  V1.71 firmware is correct; both simulators dispatch BF/2N frames correctly when bytes arrive, but the V1.71 foreground busy-loop in `display_loop_iteration` exits after the first BF/2N frame and the cadence loop doesn't re-fire without continuous user-input events (per the 2026-04-28 P3.6b research closure).  Real silicon's IR/button/mute traffic exits the busy-loop every few ms; sim test scenarios with extended dwells without injected events hit this design feature.  Task #94 (2026-05-04, closed as duplicate of the 2026-04-28 closure) confirmed via probes v10/v11 with new `uart_rx_history` + per-destination RX captures: rust accepts ALL bytes at the silicon FIFO and dispatches at least one BF/2N frame just like gpsim.  The four `_V171_V32_PB2_BRIDGE_XFAIL` markers in the Python test stay in place because the no-user-events test gate is non-converging by design, not because of a sim fidelity bug.
...
| Task #22          | Pre-existing tracking task for the `chain_gpsim.py` two-MAIN echo-loop. **Status: split (2026-04-27, refined 2026-05-04)** -- the architectural bridge-mirror echo (P3.6a) is retired by the Rust silicon ring (proven by synthetic + firmware-driven probes); P3.6b (CONTROL `v171_diag_present` saturation) is open as a TEST-SCENARIO ARTIFACT (NOT a sim fidelity bug). Real silicon converges cleanly (LCD camera shows Overflow layouts on both PB1 and PB2 pages, `PBn:` colon prefix → `v171_diag_present.bit_n = 1`); V1.71 firmware is therefore correct. Both simulators dispatch BF/2N frames correctly when bytes arrive, but V1.71's foreground busy-loop in `display_loop_iteration` exits after the first BF/2N frame and the cadence loop doesn't re-fire without continuous user-input events (per the 2026-04-28 P3.6b research closure). Task #94 (closed 2026-05-04 as duplicate of the 2026-04-28 closure) confirmed via probes v10/v11 with `uart_rx_history` + per-destination RX captures: rust accepts ALL bytes at the silicon FIFO (zero loss vs MAIN1.TX) and dispatches at least one BF/2N frame just like gpsim. The four `_V171_V32_PB2_BRIDGE_XFAIL` markers stay in place because the no-user-events test gate is non-converging by design. |
