...
- Reproduce `test_v171_v32_layer5_diag_chain.py::test_v171_v32_layer5_chain_diag_page_polls_pb1_and_pb2` (the Task #22 XFAIL).  **Status (2026-04-27): split into P3.6a / P3.6b** — see `SIM_REWRITE_RUST_PROGRESS.md`.  P3.6a (architectural retirement of the bridge-mirror echo loop) is **completed** — the Rust silicon-correct ring is structurally incapable of producing the gpsim PTY-bridge mirror echo, proven by synthetic + firmware-driven probes.  P3.6b (CONTROL BF/2N reply-path processing parity, i.e. `v171_diag_present` reaching `0x03`) is **open: rust fidelity bug under investigation as task #94**.  Hardware probe ran on real DLCP rig 2026-04-27; LCD camera captures of PB1 Diag (`PB1: I+ D1 SE B4 / RA A3 O8 V6 W+..`) and PB2 Diag (`PB2: I+ D  S+ B / R+ A9  P  OB W9..`) both show the V1.71 Tier-1 Overflow layout (`PBn:` colon prefix → `v171_diag_present.bit_n = 1`), proving **both BF/2N reply convergences work on real silicon**.  V1.71 firmware is correct.  Real HW operator presses 4 RIGHT to reach PB1 Diag and the LCD converges to diag values without further input — chain comm chatter (BF/04 heartbeat, status broadcasts, cmd 0x21 cadence) is timer-driven, INDEPENDENT of IR/button traffic.  Rust does not match HW on this: probes v15-v18 (2026-05-04) show the rust chain goes SILENT post-nav (zero CTL/MAIN0/MAIN1 TX across 10M ticks ≈ 208ms wall), so V1.71's foreground busy-loop never sees `control_flags.bit3` set and cmd 0x21 cadence never re-fires.  Likely cause: Timer3/Timer1 ISR vector dispatch path on rust is not generating periodic interrupts that drive the heartbeat / cadence on real silicon.  Task #94 reopened 2026-05-04 with this diagnosis; investigation continues with timer/IRQ probing.  The four `_V171_V32_PB2_BRIDGE_XFAIL` markers in the Python test stay in place pending the rust timer/IRQ fidelity fix.
...
| Task #22          | Pre-existing tracking task for the `chain_gpsim.py` two-MAIN echo-loop. **Status: split (2026-04-27, refined 2026-05-04)** -- the architectural bridge-mirror echo (P3.6a) is retired by the Rust silicon ring (proven by synthetic + firmware-driven probes); P3.6b (CONTROL `v171_diag_present` saturation) is open as a RUST FIDELITY BUG under task #94. Real silicon converges cleanly (LCD camera shows Overflow layouts on both PB1 and PB2 pages, `PBn:` colon prefix → `v171_diag_present.bit_n = 1`); V1.71 firmware is therefore correct. Real HW shows diag values after just 4 RIGHT presses without further input -- chain comm chatter (BF/04 heartbeat, status broadcasts, cmd 0x21 cadence) is timer-driven, independent of IR/button traffic. Rust does not match: probes v15-v18 show the rust chain goes silent post-nav (zero TX from any core for 10M ticks ≈ 208ms wall), so V1.71's foreground busy-loop never re-fires cadence and LCD stays "PB1 / n/a". Task #94 was briefly closed 2026-05-04 as duplicate but RE-OPENED the same day after user pushback; the actual issue is most likely Timer3/Timer1 ISR vector dispatch on rust failing to drive periodic interrupts that real silicon produces continuously. The four `_V171_V32_PB2_BRIDGE_XFAIL` markers stay in place pending the rust timer/IRQ fidelity fix. |
