                                    ...
                                    const char *pProcessorName)
{
  ...
  auto select_named_processor = [this](const char *processor_type,
                                       const char *processor_name) -> Processor * {
    if (processor_name == nullptr || *processor_name == '\0') {
      return nullptr;
    }
    ...
    Processor *existing = FindProcessor(processor_name);
    if (existing != nullptr) {
      if (existing->m_pConstructorObject != pc) {
        std::cout << "processor '" << processor_name
             << "' already exists with a different type.\n";
        return nullptr;
      }
      SetActiveProcessor(processor_name);
      return existing;
    }

    return add_processor(pc, processor_name);
  };
  ...
  if (pProcessorType) {
    if (pProcessorName && *pProcessorName) {
      pProcessor = select_named_processor(pProcessorType, pProcessorName);
    } else {
    ...
    const char *processor_name = pProcessorName;
    if ((processor_name == nullptr || *processor_name == '\0') &&
        !m_DefProcessorNameNew.empty()) {
      processor_name = m_DefProcessorNameNew.c_str();
    }

    if (processor_name && *processor_name) {
      pProcessor = select_named_processor(m_DefProcessorName.c_str(), processor_name);
    } else {
      pProcessor = SetProcessorByType(m_DefProcessorName.c_str(), nullptr);
    }

    if (pProcessor) {
      bReturn  = pProcessor->LoadProgramFile(filename, pFile, processor_name);
    }
...
Processor * CSimulationContext::FindProcessor(const char *processor_name)
{
  if (processor_name == nullptr) {
    return nullptr;
  }

  auto it = processor_list.find(processor_name);
  return it == processor_list.end() ? nullptr : it->second;
}

bool CSimulationContext::SetActiveProcessor(const char *processor_name)
{
  Processor *p = FindProcessor(processor_name);
  if (p == nullptr) {
    return false;
  }
