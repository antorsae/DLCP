...
class Config2H_K20 : public ConfigWord
{
    static constexpr unsigned int WDTEN = 1 << 0;
    static constexpr unsigned int WDTPS0 = 1 << 2;
    static constexpr unsigned int WDTPS1 = 1 << 3;
    static constexpr unsigned int WDTPS2 = 1 << 4;
    static constexpr unsigned int WDTPS3 = 1 << 5;
    static constexpr unsigned int DEFAULT_VALUE = WDTEN | WDTPS0 | WDTPS1 | WDTPS2 | WDTPS3;

public:
    Config2H_K20(_16bit_processor *cpu, unsigned int addr)
        : ConfigWord("CONFIG2H", DEFAULT_VALUE, "WatchDog configuration", cpu, addr)
    {
        set(DEFAULT_VALUE);
    }
    ...
    void set(gint64 v) override
    {
        ...
        if (m_pCpu)
        {
            int postscale = (int)((v & (WDTPS0 | WDTPS1 | WDTPS2 | WDTPS3)) >> 2);
            m_pCpu->wdt->set_postscale(postscale);
            m_pCpu->wdt->initialize((bool)(v & WDTEN));
        }
    }
};
...
//------------------------------------------------------------------------
void P18F25K20::create_iopin_map()
{
...
P18F25K20::P18F25K20(const char *_name, const char *desc)
    : _16bit_processor(_name, desc),
      ...
      comparator(this),
      cvrcon2(this, "cvrcon2", "Comparator Voltage Reference Control Register 2"),
      cvrcon(this, "cvrcon", "Comparator Voltage Reference Control Register"),
      ...
      iocb(nullptr)
{
    if (verbose)
    {
        std::cout << "18F25K20 constructor, type = " << isa() << '\n';
    }
...
P18F25K20::~P18F25K20()
{
    ...
    remove_sfr_register(&cvrcon2);
    remove_sfr_register(&cvrcon);
...
void P18F25K20::create()
{
    if (verbose)
    {
        std::cout << "P18F25K20::create\n";
    }
    ...
    m_configMemory->addConfigWord(CONFIG1H - CONFIG1L, new Config1H_4bits(this, CONFIG1H, 0xf8));
    m_configMemory->addConfigWord(CONFIG2H - CONFIG1L, new Config2H_K20(this, CONFIG2H));
    m_configMemory->addConfigWord(CONFIG3H - CONFIG1L, new Config3H(this, CONFIG3H, 0xff));
}


void P18F25K20::create_base_ports()
{
    ...
    iocb = new IOC(this, "iocb", "Interrupt-On-Change Portb Control Register", 0xff);
    m_portb = new PicPortGRegister(this, "portb", "", &intcon, iocb, 8, 0xff,
                                   &intcon2, &intcon3);
...
void P18F25K20::set_config3h(gint64 value)
{
    (value & MCLRE) ? assignMCLRPin(1) : unassignMCLRPin();
    ccp2con.setIOpin((value & CCP2MX) ? &((*m_portc)[1]) : &((*m_portb)[3]));
    anselb.por_value = (value & PBADEN) ? RegisterValue(0x1f, 0) : RegisterValue(0x00, 0);
}


void P18F25K20::osc_mode(unsigned int value)
{
    IOPIN *m_pin;
...
Processor *P18F25K20::construct(const char *name)
{
    P18F25K20 *p = new P18F25K20(name);

    if (verbose)
    {
        std::cout << " 18F25K20 construct\n";
    }
...
void P18F25K20::create_sfr_map()
{
    ...
    if (verbose)
    {
        std::cout << "create_sfr_map P18F25K20\n";
    }
    ...
    usart.setIOpin(&(*m_portc)[6], USART_MODULE::TX_PIN);
    usart.setIOpin(&(*m_portc)[7], USART_MODULE::RX_PIN);

    add_sfr_register(ssp.sspmsk, 0xf77, RegisterValue(0xff, 0), "sspmask");
    add_sfr_register(&slrcon, 0xf78, RegisterValue(0x07, 0));
    add_sfr_register(comparator.cmxcon1[0], 0xf79, porv, "cm2con1");
    add_sfr_register(comparator.cmxcon0[1], 0xf7a, porv, "cm2con0");
    add_sfr_register(comparator.cmxcon0[0], 0xf7b, porv, "cm1con0");
    add_sfr_register(wpub, 0xf7c, porvh);
    add_sfr_register(iocb, 0xf7d, porv);
    add_sfr_register(&ansela, 0xf7e, RegisterValue(0xff, 0));
    add_sfr_register(&anselb, 0xf7f, RegisterValue(0x1f, 0));

    add_sfr_register(&cvrcon2, 0xfb4, porv, "cvrcon2");
    add_sfr_register(&cvrcon, 0xfb5, porv, "cvrcon");
...
P18F14K22::P18F14K22(const char *_name, const char *desc)
    : _16bit_processor(_name, desc),
      ...
      wpua(nullptr), wpub(nullptr), ioca(nullptr), iocb(nullptr)
{
    if (verbose)
    {
        std::cout << "18F14K22 constructor, type = " << isa() << '\n';
    }
...
//RRR    set_config_word(CONFIG2H, 0x1f1f);
...
//  add_sfr_register( osccon,     0xfd3, RegisterValue(0x30,0), "osccon");
    ...
    adcon0.setA2DBits(10);
    adcon1.attach_ad_fvr(vrefcon0.get_node_cvref(), 0x1f);
    adcon1.attach_DAC(vrefcon1.get_node_dacout(), 0x1e);
...
void P18F14K22::create_base_ports()
{

    ioca = new IOC(this, "ioca", "Interrupt-On-Change Porta Control Register", 0x3f);
    m_porta = new PicPortGRegister(this,"porta","", &intcon, ioca, 8,0xff,
                                   &intcon2, &intcon3);
    ((PicPortGRegister *)m_porta)->assignRBPUSink(7,&intcon2);
    m_porta->setEnableMask(0x37);
    m_trisa = new PicTrisRegister(this,"trisa","", m_porta, false);
    m_trisa->setEnableMask(0x37);
    m_lata  = new PicLatchRegister(this,"lata","", m_porta);
    m_lata->setEnableMask(0x37);
    ...
    m_portb = new PicPortGRegister(this,"portb","", &intcon, iocb, 8, 0xf0);
    m_portb->assignRBPUSink(7,&intcon2);
    m_trisb = new PicTrisRegister(this,"trisb","", m_portb, false);
    m_latb  = new PicLatchRegister(this,"latb","", m_portb);
...
Processor * P18F14K22::construct(const char *name)
{
    ...
    if (verbose & 2)
    {
        std::cout << " 18F14K22 construct completed\n";
    }
...
{
    //RRR RegisterValue porv(0,0);
    RegisterValue porvh(0xff,0);

    if (verbose)
    {
        std::cout << "create_sfr_map P18F14K22\n";
    }

    _16bit_processor::create_sfr_map();

    remove_sfr_register(&lvdcon);
    m_trisa->value = RegisterValue (0x3f,0);
    m_trisa->por_value = RegisterValue (0x3f,0);
    m_trisa->initialize();
    m_trisb->por_value = RegisterValue (0xf0,0);
    ...
    add_sfr_register(comparator.cmxcon0[0],   0xf6d, porv, "cm1con0");
    add_sfr_register(comparator.cmxcon0[1],   0xf6b, porv, "cm2con0");
    ...
    comparator.cmxcon0[1]->setBitMask(0xbf);
    comparator.cmxcon0[1]->setIntSrc(new InterruptSource(pir2, PIR2v2::C2IF));
    comparator.attach_cda_fvr(vrefcon0.get_node_cvref());
    ...
    add_sfr_register(&usart.baudcon,  0xfb8, porv, "baudcon");
    usart.set_eusart(true);
    usart.setIOpin(&(*m_portb)[7], USART_MODULE::TX_PIN);
    usart.setIOpin(&(*m_portb)[5], USART_MODULE::RX_PIN);
    init_pir2(pir2, PIR2v4::TMR3IF);
    tmr3l.setIOpin(&(*m_portc)[0]);
...
P18F26K22::P18F26K22(const char *_name, const char *desc)
    : _16bit_processor(_name, desc),
      m_porte(nullptr), m_trise(nullptr), m_late(nullptr),
      adcon0(this, "adcon0", "A2D control register 0"),
      adcon1(this, "adcon1", "A2D control register 1"),
      adcon2(this, "adcon2", "A2D control register 2"),
      vrefcon0(this, "vrefcon0", "Fixed Voltage Reference Control Register"),
      ...
      osccon2(this, "osccon2", "Oscillator Control Register 2"),
      wpub(nullptr), iocb(nullptr)

{
    if (verbose)
    {
        std::cout << "18F26K22 constructor, type = " << isa() << '\n';
    }
...
void P18F26K22::create_base_ports()
{
...
void P18F26K22::create()
{
    ...
    wdt->set_timeout(0.004096);
    m_configMemory->addConfigWord(CONFIG2H - CONFIG1L, new Config2H_WDTEN(this, CONFIG2H));
...
Processor * P18F26K22::construct(const char *name)
{
    ...
    if (verbose & 2)
    {
        std::cout << " 18F26K22 construct completed\n";
    }
...
void P18F26K22::create_sfr_map()
{
    ...
    if (verbose)
    {
        std::cout << "create_sfr_map P18F26K22\n";
    }
    ...
    vrefcon1.attach_cda_fvr(vrefcon0.get_node_cvref());
    vrefcon1.setDACOUT(&(*m_porta)[2]);
    ccp2con.setCrosslinks(&ccpr2l, pir2, PIR2v2::CCP2IF, &tmr2);
...
//  add_sfr_register(osccon,     0xfd3, RegisterValue(0x30,0), "osccon");
    ...
    adcon0.setA2DBits(10);
    adcon1.attach_cda_fvr(vrefcon0.get_node_cvref(), 0x1f);
    adcon1.attach_DAC(vrefcon1.get_node_dacout(), 0x1e);
