...
FileStimulus::FileStimulus(const char *_name)
  : StimulusBase(_name,
      ...
      " .initial - idle pin voltage while waiting for a sample\n"
      " .poll - cycle interval used to poll streaming inputs\n"),
      m_future_value(0.0)
{
  // Attributes for the stimulus file/pipe.
  m_filename = new FileNameAttribute<FileStimulus>(this);
  m_initial = new Float("initial", 0.0,
      "Initial pin voltage while waiting for the first sample");
  m_poll = new Integer("poll", 16,
      "Cycle interval used to poll when the input stream has no complete sample");
  addSymbol(m_initial);
  addSymbol(m_poll);
  create_iopin_map();
...
void FileStimulus::newFile()
{
  ...
  double initial = 0.0;
  m_initial->get(initial);
  putState(initial);

  if (m_filename->getVal())
  {
#ifndef _WIN32
    m_fd = open(m_filename->getVal(), O_RDONLY | O_NONBLOCK);
    if (m_fd < 0)
    {
        std::cerr << "Warning " << name() << " cannot open " << m_filename->getVal()
...
bool FileStimulus::readLine(std::string &line)
{
#ifndef _WIN32
  while (true) {
    std::string::size_type newline = m_buffer.find('\n');
    ...
    char chunk[256];
    ssize_t count = read(m_fd, chunk, sizeof(chunk));
    if (count > 0) {
      m_buffer.append(chunk, static_cast<size_t>(count));
      continue;
    }
    ...
    std::cerr << "File Error " << name() << " " << m_filename->getVal()
              << ": " << strerror(errno) << std::endl;
    return false;
  }
#else
...
void FileStimulus::schedulePoll()
{
  gint64 poll_cycles = 1;
  m_poll->get(poll_cycles);
  if (poll_cycles < 1) {
    poll_cycles = 1;
  }

  m_future_cycle = get_cycles().get() + static_cast<guint64>(poll_cycles);
  m_polling = true;
  get_cycles().set_break(m_future_cycle, this);
}
...
void FileStimulus::parseLine(bool first)
{
  ...
  while (readLine(raw_line)) {
    ...
    guint64 next_cycle = 0;
    double next_value = 0.0;
    std::istringstream line_stream(raw_line);
    line_stream >> std::dec >> next_cycle >> next_value;
    if (line_stream.fail()) {
      std::cerr << "File Error " << name() << " " << m_filename->getVal()
                << " while parsing '" << raw_line << "'" << std::endl;
      continue;
    }
    if (verbose)
    {
      std::cout << this->name() << " read " << std::dec << next_value << " @ 0x"
           << std::hex << next_cycle << '\n';
    }

    if (next_cycle > get_cycles().get()) {
      m_future_cycle = next_cycle;
      m_future_value = next_value;
      m_polling = false;
      get_cycles().set_break(m_future_cycle, this);
      return;
    }

    // Apply the most recent state immediately when a sample arrives
    // at or before the current cycle. This keeps streaming pipes usable
    // even when the producer and consumer briefly desynchronize.
    putState(next_value);
    first = false;
  }
