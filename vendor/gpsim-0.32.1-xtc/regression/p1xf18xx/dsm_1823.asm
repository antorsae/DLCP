...
;  .sim "V1.voltage=1.0"
  ...
  .sim "period 51"
  .sim "end"
  .sim "attach cin2 carh portc5"
  .sim "attach min MIN portc3"
  .sim "attach cin1 carl portc2"
  .sim "br c 210 "
...
start
   ...
   .assert "(oscstat & 0x19) == 0x19,  \"*** FAILED 16f1823 HFIO bit error\""
	...
	goto    $
        banksel PORTA
	clrf	PORTA
   .assert "portc == 0, \"DSM 16f1823 MIN=0 CH=0 CL=0 out=0\""
	nop
	bsf	DRIVE_CIN1		; CARL = 1
   .assert "portc == 0x14, \"DSM 16f1823 MIN=0 CH=0 CL=1 out=1\""
	nop
	bsf	DRIVE_MIN		; MIN = 1
   .assert "portc == 0x0c, \"DSM 16f1823 MIN=1 CH=0 CL=1 out=0\""
	nop
	BANKSEL MDCON
	bsf	MDCARH,MDCHPOL
   .assert "portc == 0x1c, \"DSM 16f1823 MIN=1 CH=0(INV) CL=1 out=1\""
	nop
	BANKSEL PORTA
	bcf	DRIVE_MIN
   .assert "portc == 0x14, \"DSM 16f1823 MIN=0 CH=0(INV) CL=1 out=1\""
	nop
	BANKSEL MDCON
	bsf	MDCARL,MDCLPOL
   .assert "portc == 0x04, \"DSM 16f1823 MIN=0 CH=0(INV) CL=1(INV) out=0\""
	nop
	bcf	MDCARL,MDCLPOL
	bcf	MDCARH,MDCHPOL
	...
	bcf	DRIVE_CIN1
	bsf	DRIVE_CIN2
   .assert "portc == 0x20, \"DSM 16f1823 MDBIT=0 CH=1 CL=0 out=0\""
	nop
	BANKSEL MDCON
	bsf	MDCON,MDBIT
   .assert "portc == 0x30, \"DSM 16f1823 MDBIT=1 CH=1 CL=0 out=1\""
	nop
	bsf	MDCON,MDOPOL
   .assert "portc == 0x20, \"DSM 16f1823 MDBIT=1 CH=1 CL=0 out=0(INV)\""
	nop
