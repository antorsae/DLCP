	...
	list	p=16f616
        ...
        __CONFIG  _CP_OFF & _WDT_OFF &  _INTRC_OSC_NOCLKOUT &   _BOR_OFF & _MCLRE_OFF
...
start:
   ...
   .sim "node na0"
   .sim "attach na0 V3.pin porta1"	; C12IN0-
   .sim "node na1"
   .sim "attach na1 V1.pin portc1"	; C12IN1-

   .sim "node na3"
   .sim "attach na3 V2.pin porta0"	; C1IN+
  ...
  .assert  "'*** PASSED 16F616 comparator test'"
	goto	$-1
...
test_tot1:
  ...
  .assert "W != 0, 'FAILED T1 gate invert'"
    ...
    .assert "(pir1 & 1) == 1, '*** FAILED P16F616 TMR1IF set'"
	nop
...
test_compar:
  ...
  .assert "cm1con0 == 0x10, 'FAILED 16f616 cm1con0 off toggle'"
	nop
  .assert "cm2con1 == 0x00, 'FAILED 16f616 cm2con1 mirror C1OUT'"
	nop

	movlw 	(1<<C1ON)	; Enable Comparator use C1IN+, C12IN0
	movwf	CM1CON0
  .assert "cm1con0 == 0x80, 'FAILED 16f616 cm1con0 ON=1'"
	nop
	bsf	CM1CON0,C1POL	; toggle ouput polarity
  .assert "cm1con0 == 0xd0, 'FAILED 16f616 cm1con0 ON=1 POL=1'"
	nop
	bsf	CM2CON0,C1POL	; toggle ouput polarity, not ON
  .assert "cm2con0 == 0x10, 'FAILED 16f616 cm2con0 ON=0 POL=1'"
	nop
  .assert "cm2con1 == 0x80, 'FAILED 16f616 cm2con1 mirror C2OUT'"
	nop
	bsf	CM1CON0,C1OE	; C1OUT t0 RA2
   .assert "(porta & 0x04) == 0x04, 'FAILED 16f616 compare RA2 not high'"
	nop 
	bcf	CM1CON0,C1POL	; toggle ouput polarity
   .assert "(porta & 0x04) == 0x00, 'FAILED 16f616 compare RA2 not low'"
	nop
