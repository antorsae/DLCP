#!/usr/bin/env python3
"""Build the canonical V3.3 MAIN release and bump its EEPROM revision."""
...
_EEPROM_REV_RE = re.compile(
    r"(^\s*db\s+0x03,\s*0x03,\s*0x)([0-9A-Fa-f]{2})(\b.*$)",
    re.MULTILINE,
)
_RUNTIME_REV_RE = re.compile(
    r"(^\s*movlw\s+0x)([0-9A-Fa-f]{2})(\s*;\s*V3\.3_RUNTIME_EEPROM_REV\b.*$)",
    re.MULTILINE,
)
_IDENTITY_REV_HI_RE = re.compile(
    r"(^\s*movlw\s+0x)([0-9A-Fa-f]{2})(\s*;\s*V3\.3_IDENTITY_REV_HI\b.*$)",
    re.MULTILINE,
)
_IDENTITY_REV_LO_RE = re.compile(
    r"(^\s*movlw\s+0x)([0-9A-Fa-f]{2})(\s*;\s*V3\.3_IDENTITY_REV_LO\b.*$)",
    re.MULTILINE,
)


def read_v33_release_revision(asm_path: Path = V33_MAIN_ASM) -> int:
    ...
    if match is None:
        raise RuntimeError(f"V3.3 EEPROM version tuple not found in {asm_path}")
    return int(match.group(2), 16)


def _rewrite_v33_release_revision(text: str) -> tuple[str, int, int]:
    org_pos = text.find("org 0xF00000")
    if org_pos < 0:
        raise RuntimeError("EEPROM data block not found")
    match = _EEPROM_REV_RE.search(text, pos=org_pos)
    if match is None:
        raise RuntimeError("V3.3 EEPROM version tuple not found")
    old_rev = int(match.group(2), 16)
    if old_rev >= 0xFF:
        raise RuntimeError(
            f"V3.3 EEPROM revision already at 0x{old_rev:02X}; cannot bump further"
        )
    new_rev = old_rev + 1
    ...
    for regex, value in (
        ...
        if ident_match is None:
            raise RuntimeError("V3.3 identity revision literal not found")
        updated = (
...
def bump_v33_release_revision(asm_path: Path = V33_MAIN_ASM) -> tuple[int, int]:
    text = asm_path.read_text(encoding="utf-8")
    try:
        updated, old_rev, new_rev = _rewrite_v33_release_revision(text)
...
def build_v33_release(
    *,
    asm_path: Path = V33_MAIN_ASM,
    output_hex: Path = V33_MAIN_HEX,
    gpasm: str = "gpasm",
) -> tuple[int, int, Path]:
    original_text = asm_path.read_text(encoding="utf-8")
    try:
        updated_text, old_rev, new_rev = _rewrite_v33_release_revision(original_text)
    except RuntimeError as exc:
        raise RuntimeError(f"{exc} in {asm_path}") from exc
    ...
    # address lookups for `V33_MAIN_HEX` unless we roll it back.
    original_lst: bytes | None = None
    asm_modified = False
    output_hex.parent.mkdir(parents=True, exist_ok=True)
    tmpdir_obj = tempfile.TemporaryDirectory(prefix="v33_release_", dir=str(output_hex.parent))
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(
        description=(
            "Bump the canonical V3.3 EEPROM revision and assemble "
    ...
    old_rev, new_rev, output_hex = build_v33_release(gpasm=args.gpasm)
    print(
        "built canonical V3.3 release: "
