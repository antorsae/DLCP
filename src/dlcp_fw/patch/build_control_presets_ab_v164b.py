...
reconnect robustness, and explicit standby/wake IR endpoints.
...
- Firmware version policy: display/version tuple updated to 1.64b.
- IR endpoint policy:
  - keep legacy POWER toggle semantics on RC5 cmd 0x32
  - add explicit STANDBY endpoint on RC5 cmd 0x3A
  - add explicit WAKE endpoint on RC5 cmd 0x3B
"""
...
from dlcp_fw.paths import STOCK_CONTROL_HEX_V16B

from dlcp_fw.paths import FIRMWARE_PATCHED_DIR
PATCHED_CONTROL_HEX_V164B = FIRMWARE_PATCHED_DIR / "DLCP_Control_V1.64b.hex"
...
org 0x1152
    movlw 0x3F                          ; V1.64b version display
...
; V1.64b: hook parser tail (label_063) to add BF/08 handler
org 0x05D0
    goto bf08_check_stub
...
volume_indicator_stub:
    ...
    movlw 0x8F
    call 0x0066
    ; V1.64b: show "!" when DSP fault (0x01F.bit7) is set
...
; --- V1.64b: BF/08 DSP fault handler ---
bf08_check_stub:
    movlw 0x08
    cpfseq 0x02F, ACCESS              ; cmd == 0x08?
    goto bf08_not_match

    ; BF/08 handler: fault byte in 0x030
    movf 0x030, F, ACCESS             ; test data byte
    bz bf08_fault_clear

    ; Fault active: set 0x01F.bit7
    bsf 0x01F, 7, ACCESS
    goto 0x05EA                        ; label_064 (return to parser)

bf08_fault_clear:
    ; Fault cleared: clear 0x01F.bit7 + trigger resync
    btfss 0x01F, 7, ACCESS            ; was fault set?
    goto 0x05EA                        ; no -> nothing to do
    bcf 0x01F, 7, ACCESS
    ; Trigger immediate full-sync by setting counter to threshold
    movlb 0x00
    movlw 0x4E
    movwf 0x0A0, BANKED               ; full-sync fires when 0x0A0 == 0x4E
    goto 0x05EA

bf08_not_match:
    ; Not cmd=0x08: fall through to original cmd=0x1D check
    movlw 0x1D
    cpfseq 0x02F, ACCESS
    goto 0x05EA                        ; label_064
    ; Original 0x1D handler (backlight timeout)
    movf 0x0A7, W, BANKED
    subwf 0x030, W, ACCESS
    btfsc STATUS, Z, ACCESS
    goto 0x05EA
    movff 0x030, 0x0A7
    call 0x0F54                        ; function_037 (backlight apply)
    goto 0x05EA
...
def main() -> int:
    ...
    ap.add_argument(
        "--out-hex",
        type=pathlib.Path,
        default=PATCHED_CONTROL_HEX_V164B,
    ...
    # Keep CONTROL version tuple at 1.64 in EEPROM defaults.
    version_bytes = {
        0xF00070: 0x01,
        0xF00071: 0x06,
        0xF00072: 0x34,  # V1.64b
    }
    ver_written = 0
    ver_changed = 0
    ...
    print(
        ...
        "eeprom=[0xF00070..0xF00072]=[0x01,0x06,0x34]",
    )
