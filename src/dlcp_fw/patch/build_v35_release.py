#!/usr/bin/env python3
"""Build the canonical V3.5 MAIN identity release and bump its 16-bit revision."""
...
_EEPROM_REV_LO_RE = re.compile(
    r"(^\s*db\s+0x03,\s*0x05,\s*0x)([0-9A-Fa-f]{2})(\b.*$)",
    re.MULTILINE,
)
_RUNTIME_REV_LO_RE = re.compile(
    r"(^\s*movlw\s+0x)([0-9A-Fa-f]{2})(\s*;\s*V3\.5_RUNTIME_EEPROM_REV(?:_LO)?\b.*$)",
    re.MULTILINE,
)
_IDENTITY_REV_LO_HI_RE = re.compile(
    r"(^\s*movlw\s+0x)([0-9A-Fa-f]{2})(\s*;\s*V3\.5_IDENTITY_REV_LO_HI\b.*$)",
    re.MULTILINE,
)
_IDENTITY_REV_LO_LO_RE = re.compile(
    r"(^\s*movlw\s+0x)([0-9A-Fa-f]{2})(\s*;\s*V3\.5_IDENTITY_REV_LO_LO\b.*$)",
    re.MULTILINE,
)
_IDENTITY_REV_HI_HI_RE = re.compile(
    r"(^\s*movlw\s+0x)([0-9A-Fa-f]{2})(\s*;\s*V3\.5_IDENTITY_REV_HI_HI\b.*$)",
    re.MULTILINE,
)
_IDENTITY_REV_HI_LO_RE = re.compile(
    r"(^\s*movlw\s+0x)([0-9A-Fa-f]{2})(\s*;\s*V3\.5_IDENTITY_REV_HI_LO\b.*$)",
    re.MULTILINE,
)


def read_v35_release_revision(asm_path: Path = V35_MAIN_ASM) -> int:
...
def _read_identity_revision(text: str) -> int:
    nibbles = []
    for regex, label in (
        (_IDENTITY_REV_HI_HI_RE, "V3.5 identity revision high-byte high nibble"),
        (_IDENTITY_REV_HI_LO_RE, "V3.5 identity revision high-byte low nibble"),
        (_IDENTITY_REV_LO_HI_RE, "V3.5 identity revision low-byte high nibble"),
        (_IDENTITY_REV_LO_LO_RE, "V3.5 identity revision low-byte low nibble"),
    ):
...
def _rewrite_v35_release_revision(text: str) -> tuple[str, int, int]:
    ...
    if match is None:
        raise RuntimeError("V3.5 EEPROM version tuple not found")
    ...
    updated = _rewrite_match(
        ...
        _RUNTIME_REV_LO_RE,
        new_lo,
        "V3.5 runtime EEPROM revision low byte",
    )
    for regex, value in (
        ...
        updated = _rewrite_match(updated, regex, value, "V3.5 identity revision nibble")
    return updated, old_rev, new_rev


def bump_v35_release_revision(asm_path: Path = V35_MAIN_ASM) -> tuple[int, int]:
    text = asm_path.read_text(encoding="utf-8")
    try:
        updated, old_rev, new_rev = _rewrite_v35_release_revision(text)
    except RuntimeError as exc:
        raise RuntimeError(f"{exc} in {asm_path}") from exc
...
def build_v35_release(
    *,
    asm_path: Path = V35_MAIN_ASM,
    output_hex: Path = V35_MAIN_HEX,
    gpasm: str = "gpasm",
) -> tuple[int, int, Path]:
    original_text = asm_path.read_text(encoding="utf-8")
    try:
        updated_text, old_rev, new_rev = _rewrite_v35_release_revision(original_text)
    except RuntimeError as exc:
        raise RuntimeError(f"{exc} in {asm_path}") from exc
    ...
    # address lookups for `V35_MAIN_HEX` unless we roll it back.
    original_lst: bytes | None = None
    asm_modified = False
    output_hex.parent.mkdir(parents=True, exist_ok=True)
    tmpdir_obj = tempfile.TemporaryDirectory(prefix="v35_release_", dir=str(output_hex.parent))
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(
        description=(
            "Bump the canonical V3.5 16-bit release revision and assemble "
    ...
    old_rev, new_rev, output_hex = build_v35_release(gpasm=args.gpasm)
    print(
        "built canonical V3.5 release: "
