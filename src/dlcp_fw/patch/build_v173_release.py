#!/usr/bin/env python3
"""Build the canonical V1.73 CONTROL release and bump its release revision."""
...
from dlcp_fw.paths import V173_CONTROL_ASM, V173_CONTROL_HEX
from dlcp_fw.sim.v17_symbols import assemble_v17


_RELEASE_REV_RE = re.compile(
    r"(^\s*db\s+0x01,\s*0x07,\s*0x33,\s*0x)([0-9A-Fa-f]{2})(\b.*$)",
    re.MULTILINE,
)
...
def read_v173_release_revision(asm_path: Path = V173_CONTROL_ASM) -> int:
    ...
    if match is None:
        raise RuntimeError(f"V1.73 release-revision tuple not found in {asm_path}")
    return int(match.group(2), 16)


def _rewrite_v173_release_revision(text: str) -> tuple[str, int, int]:
    marker = text.find("control_release_metadata:")
    if marker < 0:
        raise RuntimeError("control_release_metadata block not found")
    match = _RELEASE_REV_RE.search(text, pos=marker)
    if match is None:
        raise RuntimeError("V1.73 release-revision tuple not found")
    old_rev = int(match.group(2), 16)
    if old_rev >= 0xFF:
        raise RuntimeError(
            f"V1.73 release revision already at 0x{old_rev:02X}; cannot bump further"
        )
    new_rev = old_rev + 1
...
def _rewrite_v173_build_date(text: str, build_date: str) -> str:
    marker = text.find("control_release_metadata:")
    if marker < 0:
        raise RuntimeError("control_release_metadata block not found")
    match = _BUILD_DATE_RE.search(text, pos=marker)
    if match is None:
        raise RuntimeError("V1.73 build-date metadata line not found")
...
def _rewrite_v173_banner_row2(text: str, revision: int, build_date: str) -> str:
    ...
    if match is None:
        raise RuntimeError("V1.73 release banner row 2 db line not found")
...
def _rewrite_v173_release_metadata(
    text: str,
    *,
    build_date: str,
) -> tuple[str, int, int]:
    updated, old_rev, new_rev = _rewrite_v173_release_revision(text)
    updated = _rewrite_v173_build_date(updated, build_date)
    updated = _rewrite_v173_banner_row2(updated, new_rev, build_date)
    return updated, old_rev, new_rev


def bump_v173_release_revision(
    asm_path: Path = V173_CONTROL_ASM,
    *,
    ...
    try:
        updated, old_rev, new_rev = _rewrite_v173_release_metadata(
            text,
...
def build_v173_release(
    *,
    asm_path: Path = V173_CONTROL_ASM,
    output_hex: Path = V173_CONTROL_HEX,
    gpasm: str = "gpasm",
    build_date: str | None = None,
) -> tuple[int, int, Path]:
    ...
    release_date = _validate_build_date(build_date or date.today().strftime("%Y%m%d"))
    try:
        updated_text, old_rev, new_rev = _rewrite_v173_release_metadata(
            original_text,
            build_date=release_date,
        )
    except RuntimeError as exc:
    ...
    # cannot leave a partial listing beside the source.  V1.73 does not
    ...
    tmpdir_obj = tempfile.TemporaryDirectory(prefix="v173_release_", dir=str(output_hex.parent))
    ...
    try:
        ...
        assert_targets_safe(["control-v173"])
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(
        description=(
            "Bump the canonical V1.73 CONTROL release revision and assemble "
    ...
    old_rev, new_rev, output_hex = build_v173_release(
        gpasm=args.gpasm,
