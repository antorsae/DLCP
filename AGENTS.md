...
- Control V1.73 (V1.72 + refactoring release): `firmware/patched/releases/DLCP_Control_V1.73.hex`
  - Current CONTROL release line when paired with V3.5 MAIN; `scripts/flash_control_safe.sh` defaults to this canonical V1.73 hex.  As of rev `0x53`, this artifact is non-hardware gated for persistent PB2 input settings plus the previous BUG-V173-MPB-PB2-DOWN-RAW and BF/08 ACKSTAT-only stale-indicator fixes, but still requires live PB2 DOWN, audio-routing, and persistence field gates before hardware field closure.
...
- `.venv/bin/python scripts/build_v173_release.py` (2026-06-22, before docs/test-only follow-up edits) -> canonical `DLCP_Control_V1.73.hex` rebuilt for persistent PB2 input settings; current non-hardware-gated identity is `V1.73 / rev 0x53 / build 20260622`, HEX SHA-256 `3a7dd25e29c3ce731a2783d1370fb5f2b387ba4a4be7c6a48b3bb19dfb207302`.  Gates passed: `tests/sim/test_v173_multi_pb_input_selection.py -q -n 8` -> `111 passed`; `scripts/check_ram_access_safety.py --target control-v173` -> OK; `tests --collect-only -q` -> `2003 tests collected`; `tests/sim -n 16 -q` -> `1978 passed, 2 skipped, 4 xfailed, 1 warning`; `scripts/check_phase5_gate.py` -> `P5.gate GREEN`; `scripts/check_gpsim_excision.py` -> clean.  Live PB2 DOWN, audio-routing, and persistence field gates were not run.
...
- V1.73 is the current CONTROL release line when paired with V3.5 MAIN and remains backward compatible with V3.4/V3.3/V3.2 staged or rollback combinations covered by the V3.4/V3.5 compatibility and identity tests.  The current rev `0x53` PB2 persistence candidate is non-hardware gated; live PB2 DOWN, audio-routing, and persistence field gates remain required before claiming hardware field closure.
