...
- Control V1.73 (V1.72 + refactoring release): `firmware/patched/releases/DLCP_Control_V1.73.hex`
  - Current CONTROL release line when paired with V3.5 MAIN; `scripts/flash_control_safe.sh` defaults to this canonical V1.73 hex.  As of rev `0x54`, this artifact is non-hardware gated for persistent PB2 input settings, fixed RC5 F4/F5 preset/input toggle shortcuts, plus the previous BUG-V173-MPB-PB2-DOWN-RAW and BF/08 ACKSTAT-only stale-indicator fixes, but still requires live PB2 DOWN, audio-routing, persistence, and IR field gates before hardware field closure.
...
Recent verification (latest 2026-06-22):

- `.venv_ep0/bin/python scripts/build_v35_release.py` -> canonical `DLCP_Firmware_V3.5.hex` built with release rev `0x008F -> 0x0090` for the TBLPTR-safe EEPROM walker bugfix. Focused gates passed: `tests/sim/test_memory_corruption_instrumentation.py` -> `8 passed in 164.26s`; `scripts/memory_corruption_trace.py --expect-clean` -> first violation `none`, corrupt preset-B units `none`; V3.5 release/identity focused tests -> `11 passed in 21.17s`. Current `DLCP_Firmware_V3.5.hex` reports EEPROM tuple `3.5 / rev 0x90` and cmd `0x25` identity rev `0090`.
...
- V1.73 is the current CONTROL release line when paired with V3.5 MAIN and remains backward compatible with V3.4/V3.3/V3.2 staged or rollback combinations covered by the V3.4/V3.5 compatibility and identity tests.  The current rev `0x54` PB2 persistence + RC5 F4/F5 shortcut candidate is non-hardware gated; live PB2 DOWN, audio-routing, persistence, and IR field gates remain required before claiming hardware field closure.
