...
Recent verification (latest 2026-06-29):
...
- V1.73 is the current CONTROL release line when paired with V3.5 MAIN and remains backward compatible with V3.4/V3.3/V3.2 staged or rollback combinations covered by the V3.4/V3.5 compatibility and identity tests.  The current rev `0x5C` PB1/PB2 persistence + RC5 F4/F5 shortcut + real-RB5 POWER-wake recovery + LCD refresh-budget + asleep-preset host-echo isolation + test-robustness candidate is non-hardware gated; live PB2 DOWN, audio-routing, persistence, IR, and test-robustness field gates remain required before claiming hardware field closure.
