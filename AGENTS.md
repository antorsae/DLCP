...
- Control V1.73 (V1.72 + refactoring release): `firmware/patched/releases/DLCP_Control_V1.73.hex`
  - Current CONTROL release line when paired with V3.5 MAIN; `scripts/flash_control_safe.sh` defaults to this canonical V1.73 hex.  As of rev `0x57` / build `20260627`, this artifact is non-hardware gated for persistent PB1/PB2 input settings, fixed RC5 F4/F5 preset/input toggle shortcuts, plus the previous BUG-V173-MPB-PB2-DOWN-RAW and BF/08 ACKSTAT-only stale-indicator fixes, but still requires live PB2 DOWN, audio-routing, persistence, IR, and test-robustness field gates before hardware field closure.
...
- V1.73 is the current CONTROL release line when paired with V3.5 MAIN and remains backward compatible with V3.4/V3.3/V3.2 staged or rollback combinations covered by the V3.4/V3.5 compatibility and identity tests.  The current rev `0x57` PB1/PB2 persistence + RC5 F4/F5 shortcut + test-robustness candidate is non-hardware gated; live PB2 DOWN, audio-routing, persistence, IR, and test-robustness field gates remain required before claiming hardware field closure.
