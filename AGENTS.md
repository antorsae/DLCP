...
- Main MCU: PIC18F2455-class firmware image (`V2.3` stock, patched `V2.4`–`V2.8`, source-assembled `V3.0`/`V3.1`/`V3.2`/`V3.3`/`V3.4`/`V3.5`)
- Control MCU: PIC18F25K20 firmware image (`V1.4`/`V1.5b`/`V1.6b` stock, patched `V1.41`/`V1.51b`/`V1.61b`/`V1.62b`/`V1.63b`/`V1.64b`, source-assembled `V1.7x`)
- DSP: TI TAS3108
...
│   ├── SIMULATION.md
│   ├── TEST_INCIDENTS.md
│   ├── TEST_ROBUSTNESS_SPEC.md
│   ├── TEST_ROBUSTNESS_IMPL.md
│   ├── TEST_SIMULATOR.md
...
- Main V3.3 (V3.2 + Diagnostics MAIN identity reply): `firmware/patched/releases/DLCP_Firmware_V3.3.hex`
  - Previous recommended MAIN release when paired with V1.72 CONTROL for PB1/PB2 Diagnostics MAIN version/revision display.
...
- Main V3.4 (V3.3 + refactoring release): `firmware/patched/releases/DLCP_Firmware_V3.4.hex`
  - Previous recommended MAIN release when paired with V1.73 CONTROL; keeps V3.3 diagnostics identity and adds RAM-bank safety hardening, preset-LCD lifecycle fixes, chain-TX arbitration, and I2C recovery classification.
  - Canonical build path: `scripts/build_v34_release.py` bumps the EEPROM revision byte at `eeprom_data[0x82]`, updates the boot-time runtime identity literal, updates the cmd `0x25` identity revision nibbles, runs RAM-bank safety for `main-v34`, then assembles/publishes the canonical filename. Do not mint ad-hoc suffixed release names for V3.4.
- Main V3.5 (V3.4 source-line promotion): `firmware/patched/releases/DLCP_Firmware_V3.5.hex`
  - Current MAIN release line for the V1.73 CONTROL candidate; source lives in `src/dlcp_fw/asm/dlcp_main_v35.asm` so V3.4 remains a stable historical source/release line.
  - As of rev `0x009B`, V3.5 fails fast on unarmed CONTROL flash relay sessions with `42 12 ...`, keeps the final `0x77BF` CONTROL app byte inside the flashable window, flushes the pending `0x77B0` metadata record before final `0x41` verification, does not inject HID-packet-boundary CR/LF into partial downstream Intel HEX records, and includes the 2026-07-02 Fable fixes for duplicate wake idempotence, channel-6 boot-source validation, fixed-input `cmd 0x06` idempotence, and RX-ring/OERR interrupt safety.
  - Canonical build path: `scripts/build_v35_release.py` bumps the EEPROM revision byte at `eeprom_data[0x82]`, updates the boot-time runtime identity literal, updates the cmd `0x25` identity revision nibbles, runs RAM-bank safety for `main-v35`, then assembles/publishes the canonical filename. Do not mint ad-hoc suffixed release names for V3.5.
...
- Control V1.72 (V1.71 + Diagnostics MAIN identity display): `firmware/patched/releases/DLCP_Control_V1.72.hex`
  - Previous recommended CONTROL release when paired with V3.3 MAIN for PB1/PB2 Diagnostics MAIN version/revision display.
  - Canonical build path: `scripts/build_v172_release.py` bumps `control_release_metadata[11]`, bakes the build date into `control_release_metadata[12..15]` plus the LCD boot splash, then assembles the release back into the same canonical filename.
- Control V1.73 (V1.72 + refactoring release): `firmware/patched/releases/DLCP_Control_V1.73.hex`
  - Current CONTROL release line when paired with V3.5 MAIN; `scripts/flash_control_safe.sh` defaults to this canonical V1.73 hex.  As of rev `0x63` / build `20260702`, this artifact is non-hardware gated for PB2 full-table row-to-cmd06 intent persistence, real-RB5 POWER-wake RC5 decoder recovery, persistent PB1/PB2 input settings, fixed RC5 F4/F5 preset/input toggle shortcuts with F4 held-repeat suppression and F5 saturated-TX no-op behavior, LCD refresh-budget reduction, asleep-preset host-echo isolation, V3.4+ Diagnostics identity rev16 parsing, cold-WAITING ISR scratch isolation, plus the previous BUG-V173-MPB-PB2-DOWN-RAW and BF/08 ACKSTAT-only stale-indicator fixes, but still requires live PB2 DOWN, audio-routing, persistence, IR, and test-robustness field gates before hardware field closure.
  - Canonical build path: `scripts/build_v173_release.py` bumps `control_release_metadata[11]`, bakes the build date into `control_release_metadata[12..15]` plus the LCD boot splash, runs RAM-bank safety for `control-v173`, then assembles/publishes the canonical filename.
- Source: `src/dlcp_fw/asm/dlcp_main_v30.asm`, `src/dlcp_fw/asm/dlcp_main_v31.asm`, `src/dlcp_fw/asm/dlcp_main_v32.asm`, `src/dlcp_fw/asm/dlcp_main_v33.asm`, `src/dlcp_fw/asm/dlcp_main_v34.asm`, `src/dlcp_fw/asm/dlcp_main_v35.asm`, `src/dlcp_fw/asm/dlcp_control_v17.asm`, `src/dlcp_fw/asm/dlcp_control_v171.asm`, `src/dlcp_fw/asm/dlcp_control_v172.asm`, `src/dlcp_fw/asm/dlcp_control_v173.asm`
...
- Reference docs: `firmware/reference/{DLCP-datasheet-R3.pdf,dlcp.md,DLCP-manual-R3.pdf,tas3108.pdf,tas3108.md,sleu067a.pdf,DLCP-control-intro.pdf,39632e.pdf,39632e.md,40001303h.pdf,40001303h.md,src4382.pdf,src4382.md}`
  ...
  - For the SRC4382 datasheet, `src4382.pdf` is authoritative. Use `src4382.md` as the line-stable converted companion when repo citations are needed.
...
- Source-assembled main: `V30_MAIN_HEX`, `V30_MAIN_ASM`, `V30_MAIN_ASM_COMMENTS`, `V31_MAIN_HEX_CANONICAL`, `V31_MAIN_ASM_CANONICAL`, `V31_MAIN_HEX`, `V31_MAIN_ASM`, `V32_MAIN_HEX`, `V32_MAIN_ASM`, `V33_MAIN_HEX`, `V33_MAIN_ASM`, `V34_MAIN_HEX`, `V34_MAIN_ASM`, `V35_MAIN_HEX`, `V35_MAIN_ASM`
...
- Source-assembled control: `V17_CONTROL_ASM`, `V17_CONTROL_ASM_COMMENTS`, `V17_CONTROL_ASM_SHIFTED`, `V171_CONTROL_ASM`, `V171_CONTROL_HEX`, `V172_CONTROL_ASM`, `V172_CONTROL_HEX`, `V173_CONTROL_ASM`, `V173_CONTROL_HEX`
  ...
  - `V173_CONTROL_HEX` is the canonical V1.73 release artifact at `firmware/patched/releases/DLCP_Control_V1.73.hex`
...
- V3.4 MAIN refactoring release source: `dlcp_main_v34.asm` (V3.3 lineage with V3.4 identity/build plumbing, RAM-bank safety hardening, Preset lifecycle fixes, chain-TX arbitration, and I2C recovery contracts tracked by `docs/REFACTORING_V34_V173_SPEC.md`)
- V3.5 MAIN promoted source: `dlcp_main_v35.asm` (V3.4 lineage with V3.5 identity/build plumbing; keep V3.4 source unchanged for historical/reproducible V3.4 rebuilds)
...
- V1.73 CONTROL refactoring release source: `dlcp_control_v173.asm` (V1.72 lineage with V1.73 release metadata/build plumbing and RAM-bank alias cleanup tracked by `docs/REFACTORING_V34_V173_SPEC.md`)
...
- Canonical V3.4 operator wrapper: `dlcp_v34_release_flash.py`
- Canonical V3.5 operator wrapper: `dlcp_v35_release_flash.py`
...
- V3.5 SRC4382 USB cmd 0x45 signal diagnostics reader: `dlcp_src4382_diag.py`
  (operator runbook: `scripts/dlcp_src4382_diag.py`; spec: `docs/PROPOSAL_1_SRC4382_USB_DIAGNOSTICS_SPEC.md`)
...
Current suite (1959 tests collected after PF.4 phase 1 deletions and current
V1.72/V3.3 Diagnostics identity, Preset filename LCD, V3.4/V1.73, and
V3.5/V1.73 release additions, plus V3.5 SRC4382 USB diagnostics, per
...
- `tests/hardware/test_live_state_transitions.py` (19 live tests: MAIN V3.2 release identity + A/B filename RAM confirmation; release-flash settings preservation; SRC4382 Auto Detect acoustic confirmation; physical front-panel A/B confirmation; preset filename LCD reconstruction/scroll confirmation; physical front-panel STBY/WAKE confirmation; preset convergence; rapid-toggle convergence; preset→mute timing sweep; preset→standby/wake timing sweep; reconnect responsiveness soak; IR receiver profile sweep; real-IR legacy stress for V1.6b-vs-V1.71 comparison; and the V1.71+V3.2 Layer 5 PB1/PB2 Diagnostics gates on the real DLCP rig — release identity gates on `DLCP_HW_RELEASE_IDENTITY_CONFIRM=1`; settings preservation gates on `DLCP_HW_RELEASE_SETTINGS_CONFIRM=1` plus `DLCP_HW_EXPECTED_VOLUME_LOW`, `DLCP_HW_EXPECTED_INPUT`, and `DLCP_HW_EXPECTED_SETUP_PROFILE`; SRC4382 acoustic confirmation gates on `DLCP_HW_SRC4382_AD_ACOUSTIC_CONFIRM=1`, `DLCP_HW_SRC4382_FIXED_INPUT_AUDIO_OK=1`, `DLCP_HW_SRC4382_AUTODETECT_AUDIO_OK=1`, `DLCP_HW_SRC4382_USER_ACTIONS_OK=1`, and `DLCP_HW_SRC4382_SOAK_OK=1`; the front-panel preset gate uses `DLCP_HW_FRONT_PANEL_PRESET_CONFIRM=1 DLCP_HW_EXPECTED_PRESET=A|B`; the front-panel standby/wake gate uses `DLCP_HW_FRONT_PANEL_STBY_WAKE_CONFIRM=1`; the IR receiver sweep gates on `DLCP_HW_IR_RECEIVER_SWEEP=1`; the cross-version IR stress gate uses `DLCP_HW_IR_LEGACY_STRESS=1`; the diag tests gate on `DLCP_HW_LAYER5_AT_DIAG=1` after the operator manually navigates CONTROL to PB1/PB2 Diag and waits for the static cadence; the diag physical-button test additionally gates on `DLCP_HW_LAYER5_BUTTON_ACTIONS=1`, and the diag IR-actions test additionally gates on `DLCP_HW_LAYER5_IR_ACTIONS=1`, with `DLCP_HW_EXPECTED_DIAG_PAGE=PB1|PB2` for per-page runs; see `docs/HARDWARE_TEST.md` §"Diagnostics page" for the full operator walk-through)
...
- `test_v171_release_banner.py` (boot LCD splash renders baked `Firmware V1.71` plus `Rev xNN YYYYMMDD`)
...
V3.5 SRC4382 USB diagnostics:
- `test_dlcp_src4382_diag.py` (host parser/CLI guards plus V3.5 MAIN USB cmd `0x45` pending/OK, optional ratio/PC-PD, unsupported page-1/cmd `0x46`, and I2C NACK status behavior)
...
V3.4/V1.73 historical refactoring release:
- `test_v34_v173_release_builders.py` (V3.4/V1.73 builders update identity metadata, run RAM-bank safety before publishing HEX, and roll back source/listing/old HEX on assembly or RAM-safety failure)
...
- `test_v173_wake_responsiveness.py` (standby->wake to responsive display under a 160M-tick bound on the native V1.73+V3.4 chain, first post-wake IR key works, the connected-WAITING exit-bound contract — WAITING must clear within 40M ticks of both MAIN gates being up, two cycles — and a structural pin that the WAITING entries never re-grow an open-loop banner delay)
...
- `test_v34_detect_cycle_volume_excursion.py` (SAFETY regression for the live 2026-06-12 ~1 s loud-audio burst: deterministic Auto-Detect loss/re-detect cycles must never write a louder-than-set TAS 0x30 master volume; pins the rev 0x87 per-route trim-selector fix)
- `test_v34_autodetect_loss_debounce.py` (rev 0x88 K=6 loss debounce: a ~1 s RXCKR status blip must not confirm a source loss or drop the held route; sustained absence still confirms and re-detects)
- `test_ram_bank_safety.py` covers old+new RAM-safety targets: `main-v33`, `control-v172`, `main-v34`, `main-v35`, and `control-v173`.
...
- `test_dlcp_control_flash_safety.py` covers CONTROL-through-MAIN flash stream construction, unarmed relay fail-fast `42 12`, old false-ACK mutation proof, and slow full-chain simulated CONTROL bootloader flashes from current-bad to fixed-good and fixed-good to newer-good.

Version labels:
- `test_firmware_version_label.py` (USB HID + EEPROM version bytes in HEX)

Recent verification (latest 2026-07-02):

- 2026-07-02 Fable confirmed-bug fix verification: `PYTHONPATH=src .venv_ep0/bin/python scripts/build_v35_release.py` rebuilt canonical `DLCP_Firmware_V3.5.hex` at EEPROM rev `0x009B`, SHA-256 `7238d08cacf32f25358cf1a83d86984cb7c1d454ce46051bafe56acc3eed1071`, with duplicate wake idempotence, channel-6 boot-source validation, fixed-input `cmd 0x06` idempotence, and RX-ring prior-`GIE` masking. `PYTHONPATH=src .venv_ep0/bin/python scripts/build_v173_release.py` rebuilt canonical `DLCP_Control_V1.73.hex` at rev `0x63` / build `20260702`, SHA-256 `9a28543e99ff1806a470826283323e9438a29dd6a4aa6917a27152a1631c2ee1`, with V3.4+ Diagnostics rev16 parsing and cold-WAITING ISR scratch isolation. Focused Fable regressions -> `25 passed in 30.30s`; affected multi-PB/Diagnostics/refactoring/field repro group -> `339 passed in 1346.10s`; release/preflight gate -> `105 passed, 3 warnings in 60.06s`; RAM safety for `main-v35` and `control-v173` -> OK; full simulator gate `PYTHONPATH=src .venv_ep0/bin/python -m pytest tests/sim -n 32 -q` -> `2175 passed, 2 skipped, 2 xfailed, 7 warnings in 1610.69s`. Live hardware was not run.

- 2026-06-30 full simulator gate after MAIN relay packet-boundary fix: `PYTHONPATH=src .venv_ep0/bin/python scripts/build_v35_release.py` rebuilt canonical `DLCP_Firmware_V3.5.hex` at EEPROM rev `0x009A`, SHA-256 `7d84601e588df6840c9f1d5d849cc7b74eaa9d0b07ec7c9f9c2c8487adfeb157`, with the CONTROL-flash relay returning after each 30-byte HID payload instead of falling through into the CR/LF emitter. CONTROL V1.73 is rev `0x62` / build `20260630`, SHA-256 `5b1c5bf41ade024a6fdad1df8715a7952e9be630d64be7445a71b0c45e684b4a`. Focused slow full-chain relay simulations with route-level complete `:10` Intel HEX assertions -> `2 passed, 32 deselected in 49.37s`; full simulator gate `PYTHONPATH=src .venv_ep0/bin/python -m pytest tests/sim -n 32 -q` -> `2150 passed, 2 skipped, 2 xfailed, 7 warnings in 991.13s`. Live hardware was not run.
- 2026-06-30 CONTROL flash relay fix verification: `PYTHONPATH=src .venv_ep0/bin/python scripts/build_v35_release.py` rebuilt canonical `DLCP_Firmware_V3.5.hex` at EEPROM rev `0x0099`, SHA-256 `fcc882e9ef1ec7cd0c5923530cd7a8e4e63c893a02bf08e418b575fb0ca76e92`, with unarmed relay fail-fast `42 12`, final `0x77BF`/`0x77C0` boundary flush, and relay-session cleanup. CONTROL V1.73 stayed at rev `0x60` / build `20260630`, SHA-256 `6bc5abd6a8ea9fc96360c6f99cd524ba3f84dcc425ae0d1d1cbb15505b670875`. Focused gates: `tests/sim/test_dlcp_control_flash_safety.py -m "not slow"` -> `32 passed`; `tests/sim/test_v34_v173_refactoring_contracts.py -k "relay or listing_size"` -> `3 passed`; slow full-chain relay simulations -> `2 passed`; Rust release simulator gate `cargo test --release -p dlcp-sim` -> passed; RAM safety for `main-v35` -> OK; full all-tests gate `tests -n 32 -q` -> `2144 passed, 21 skipped, 2 xfailed, 7 warnings in 1302.84s`. Live hardware was not run.
- 2026-06-30 current-line fix verification: `PYTHONPATH=src .venv_ep0/bin/python scripts/build_v35_release.py` rebuilt canonical `DLCP_Firmware_V3.5.hex` at EEPROM rev `0x0095`, SHA-256 `fdff18de3cbefe5f9ddd94f4f26c8ba9456d3568d36d23e71b533c5e78c4a6b5`, with `chain_copy` preserving prior `GIE` around the `TOSL/TOSH` commit. `PYTHONPATH=src .venv_ep0/bin/python scripts/build_v173_release.py` rebuilt canonical `DLCP_Control_V1.73.hex` at rev `0x60` / build `20260630`, SHA-256 `6bc5abd6a8ea9fc96360c6f99cd524ba3f84dcc425ae0d1d1cbb15505b670875`, with PB2 full-table row-to-cmd06 intent persistence fixed. Focused gates: release/identity/IR/chain-copy/FIELD-10 -> `13 passed in 29.09s`; PB2 all-row mapping subset -> `10 passed in 122.44s`; RAM safety for `main-v35` and `control-v173` -> OK; full all-tests gate `tests -n 32 -q` -> `2135 passed, 21 skipped, 2 xfailed, 7 warnings in 1662.40s`. Live hardware was not run.
- 2026-06-30 IR F4/F5 hardening verification: `PYTHONPATH=src .venv_ep0/bin/python scripts/build_v173_release.py` rebuilt canonical `DLCP_Control_V1.73.hex` at rev `0x61` / build `20260630`, SHA-256 `e23ef3cd3c90829910740564cac86f22e1f7ca8f8946e72bfe0e8f308bf5db2f`, with F4 held-repeat suppression and F5 pre-mutation TX capacity checks for single-known, independent, and linked PB1/PB2 routing. Focused gates: F4/F5 source-assembled multi-PB slice -> `29 passed, 149 deselected in 99.05s`; real-RB5 F4 held-repeat and F4/F5 receiver smoke on source+canonical -> `4 passed, 18 deselected in 18.25s`; V1.73/V3.5 release builder parity -> `14 passed in 0.06s`; V1.71/V1.73 dispatcher + Flipper sender compatibility -> `28 passed in 117.03s`; RAM safety for `control-v173` -> OK. Full all-tests and live hardware were not run for this x61 hardening pass.
- 2026-06-29 verification of the 2026-06-28 BUG-IR-POWER-WAKE-RC5-DEAD + LCD/PB2/Field-8 follow-up CONTROL build: `PYTHONPATH=src .venv_ep0/bin/python scripts/build_v173_release.py` rebuilt canonical `DLCP_Control_V1.73.hex` at rev `0x5C` / build `20260628`, SHA-256 `04223d7b6f677671431cef3fac6e1b39986b3f5041e95a7eab722a91c96cdb4f`. Focused IR/reconnect/release/LCD/PB2/Field-8 gates passed on source/canonical slices; full all-tests gate `tests -n 16 -q` -> `2111 passed, 21 skipped, 4 xfailed, 7 warnings in 1589.30s`. Live hardware was not run.
- 2026-06-27 test-robustness docs/tests pass on then-current canonical artifacts: MAIN V3.5 EEPROM rev `0x91` (`DLCP_Firmware_V3.5.hex` SHA-256 `2e17a79dfd0686d95559275d70b2d830cf40de3dda4f61984c3a8b7b40819f7e`) and CONTROL V1.73 rev `0x57` / build `20260627` (`DLCP_Control_V1.73.hex` SHA-256 `27fd91c6f0b09bed8e05268b7a5f2ce370e994290ce7840e1897856f20a4e88a`).  Gates passed: focused robustness gate -> `55 passed in 75.48s`; current release-artifact middle gate -> `67 passed in 0.86s`; `scripts/check_ram_access_safety.py --target main-v35` -> OK; `scripts/check_ram_access_safety.py --target control-v173` -> OK; full simulator gate `tests/sim -n 16 -q` -> `2082 passed, 2 skipped, 4 xfailed, 10 warnings in 1001.77s`.  Live hardware was not run for this docs/tests-only pass.
- Previous V3.5 TBLPTR-safe EEPROM walker verification rebuilt `DLCP_Firmware_V3.5.hex` to EEPROM tuple `3.5 / rev 0x91` and cmd `0x25` identity rev `0091`.  The earlier walker bugfix build moved rev `0x008F -> 0x0090` with focused gates: `tests/sim/test_memory_corruption_instrumentation.py` -> `8 passed in 164.26s`; `scripts/memory_corruption_trace.py --expect-clean` -> first violation `none`, corrupt preset-B units `none`; V3.5 release/identity focused tests -> `11 passed in 21.17s`.
- `.venv/bin/python scripts/build_v35_release.py` -> canonical `DLCP_Firmware_V3.5.hex` built with release rev `0x0083 -> 0x0084`; follow-up direct reassembly after local helper-label cleanup kept rev `0x0084` and RAM safety passed for `main-v35`. The filename-EEPROM NUL fix later rebuilt canonical V3.5 with release rev `0x0084 -> 0x0085`; the SRC4382 USB diagnostics work bumped through `0x0086` to `0x0088`; the compact raw-read V1a rebuilt canonical V3.5 with release rev `0x008E -> 0x008F`.
- `.venv_ep0/bin/python -m pytest tests/sim/test_dlcp_src4382_diag.py -q` -> `65 passed in 38.85s`; `.venv_ep0/bin/python -m pytest tests/sim/test_dlcp_diag.py tests/sim/test_v35_v173_release_builders.py -q` -> `54 passed, 3 warnings in 0.21s`; `.venv_ep0/bin/python -m pytest -q tests/sim/test_ram_bank_safety.py` -> `19 passed in 0.93s`; `.venv_ep0/bin/python scripts/check_ram_access_safety.py --target main-v35` -> `RAM bank safety: OK (main-v35)`; `cargo test -p dlcp-sim --release` -> passed.
- Previous adjacent SRC gates: `.venv_ep0/bin/python -m pytest tests/sim/test_v32_src4382_autodetect_polling.py -q` -> `51 passed in 72.24s`; `.venv_ep0/bin/python -m pytest tests/sim/test_v32_src4382_audio_path_regression.py -q` -> `9 passed in 8.06s`.
- `.venv_ep0/bin/python -m pytest -q tests/sim/test_ram_bank_safety.py` -> `19 passed in 0.96s`.
- `.venv_ep0/bin/python -m pytest tests/sim -n 16 -q` -> `1824 passed, 2 skipped, 4 xfailed, 7 warnings in 682.97s`.
- `.venv_ep0/bin/python -m pytest tests --collect-only -q` -> `1959 tests collected in 0.59s`.
...
- `PYTHONPATH=src .venv_ep0/bin/python scripts/build_v34_release.py` (2026-06-12) -> canonical `DLCP_Firmware_V3.4.hex` rebuilt; current canonical release identity is `V3.4 / rev 0x8A` (SRC/DSP forensic counters + extended cmd 0x44; detect-cycle volume-excursion fix at the per-route trim selector; Auto-Detect loss debounce widened to 6 consecutive samples); MAIN listing margin before `0x4C00` is ~28 bytes (headroom gate floor ratcheted 96 -> 24 in `test_v34_v173_refactoring_contracts.py`; size-reclaim pass required before further MAIN features)
- `PYTHONPATH=src .venv_ep0/bin/python scripts/build_v173_release.py --build-date 20260608` -> historical canonical `DLCP_Control_V1.73.hex` rebuild at `V1.73 / rev 0x40 / build 20260608`.
- `.venv_ep0/bin/python scripts/build_v173_release.py` (2026-06-25) -> historical canonical `DLCP_Control_V1.73.hex` rebuilt for fixed RC5 F4/F5 preset/input toggle shortcuts; then-current non-hardware-gated identity was `V1.73 / rev 0x54 / build 20260625`, HEX SHA-256 `ce6aa82d4cd874c5a6a40b3d93cc2be6413cbcdf7c04553d4b9bc3ca2c378280`.  Gates passed: `tests/sim/test_v173_multi_pb_input_selection.py -q -k 'ir or input'` -> `142 passed`; `tests/sim/test_v34_v173_release_builders.py -q` -> `7 passed`; `tests/sim/test_v171_ir_command_matrix.py tests/sim/test_v171_preset_inline.py tests/sim/test_v171_ir_endpoints.py -q` -> `21 passed`; `tests/sim/test_hardware_flipper_ir.py -q` -> `7 passed`; `scripts/check_ram_access_safety.py --target control-v173` -> OK.  Live PB2 DOWN, audio-routing, persistence, and IR field gates were not run.
- `.venv/bin/python scripts/build_v173_release.py` (2026-06-22, before docs/test-only follow-up edits) -> historical canonical `DLCP_Control_V1.73.hex` rebuilt for persistent PB2 input settings; then-current non-hardware-gated identity was `V1.73 / rev 0x53 / build 20260622`, HEX SHA-256 `3a7dd25e29c3ce731a2783d1370fb5f2b387ba4a4be7c6a48b3bb19dfb207302`.  Gates passed: `tests/sim/test_v173_multi_pb_input_selection.py -q -n 8` -> `111 passed`; `scripts/check_ram_access_safety.py --target control-v173` -> OK; `tests --collect-only -q` -> `2003 tests collected`; `tests/sim -n 16 -q` -> `1978 passed, 2 skipped, 4 xfailed, 1 warning`; `scripts/check_phase5_gate.py` -> `P5.gate GREEN`; `scripts/check_gpsim_excision.py` -> clean.  Live PB2 DOWN, audio-routing, and persistence field gates were not run.
- `.venv_ep0/bin/python scripts/build_v173_release.py` (2026-06-22) -> historical canonical `DLCP_Control_V1.73.hex` rebuild for BUG-V173-MPB-PB2-DOWN-RAW, the raw-status scratch collision fix, and the BF/08 ACKSTAT-only stale-indicator follow-up; then-current non-hardware-gated identity was `V1.73 / rev 0x52 / build 20260622`, HEX SHA-256 `66ab68c47d4737fb72b6a1232ea6cd34592fab407dba6515e5a5a97906f2e5f6`. Gates passed: focused multi-PB/compat/control-flash slice -> `83 passed`; targeted broad-gate failure slice -> `6 passed`; `tests/sim -n 16` -> `1934 passed, 2 skipped, 4 xfailed, 7 warnings`; `scripts/check_phase5_gate.py` -> `P5.gate GREEN`; `scripts/check_gpsim_excision.py` -> clean. Hardware PB2 DOWN field gate was not run.
- V3.4/V1.73 listing headroom after the 2026-06-12 size-reclaim S-series (chain_copy S1/S2/S4 + S3 dedup subroutines, `docs/V34_SIZE_OPTIMIZATION_FINDINGS.md`): MAIN `app_end=0x4B06`, `byte_margin=250` (gate measure; floor ratcheted to 200), `free_object_words=125` before `0x4C00`; CONTROL `app_end=0x2DC0`, `byte_margin=18928`, `free_object_words=9464` before `0x77B0`.
- 2026-06-12 sim fidelity/harness fixes landed with the size series: rust core TOS write-through for `movwf` to TOSL/TOSH/TOSU (`tos_sw_write_pending`, pinned by `movwf_tos_write_through_patches_computed_return`), and `Chain.inject_main_frames_fifo` now defers injection while MAIN0 is mid-ISR (GIEH=0) or RCIF=1 — the un-deferred injector could race the RX ISR's cached write index and corrupt the injected route byte.
...
- `scripts/flash_control_safe.sh --preflight-only` -> historical target release `V1.73 / rev 0x40 / build 20260608`, CRC `0x25D0`, no USB writes.
- `PYTHONPATH=src .venv_ep0/bin/python -m pytest -q tests/sim/test_v34_v173_compatibility.py tests/sim/test_v172_v33_diag_identity.py::test_v173_v34_diag_ok_title_shows_visible_main_identity` -> `7 passed in 52.33s`
- `PYTHONPATH=src .venv_ep0/bin/python -m pytest -q tests/sim/test_preset_filename_lcd_spec.py::test_v173_v34_full_native_chain_filename_feature tests/sim/test_preset_filename_lcd_spec.py::test_v173_v34_full_native_chain_filename_preset_state_matrix tests/sim/test_preset_filename_lcd_spec.py::test_v173_v34_full_native_chain_filename_preset_reentry_matrix tests/sim/test_preset_filename_lcd_spec.py::test_v173_v34_full_native_chain_preset_reentry_immediate_left_never_blanks_row0 tests/sim/test_preset_filename_lcd_spec.py::test_v173_native_preset_entry_paint_precedes_filename_cache_reuse tests/sim/test_preset_filename_lcd_spec.py::test_v173_v34_full_native_chain_preset_b_survives_next_menu_standby_wake tests/sim/test_preset_filename_lcd_spec.py::test_v34_v173_refactoring_layout_labels_are_pinned` -> `17 passed in 155.76s`
- `PYTHONPATH=src .venv_ep0/bin/python scripts/check_ram_access_safety.py --target main-v33 --target control-v172 --target main-v34 --target main-v35 --target control-v173` -> `RAM bank safety: OK (main-v33, control-v172, main-v34, main-v35, control-v173)`
- `PYTHONPATH=src .venv_ep0/bin/python scripts/check_gpsim_excision.py` -> `gpsim retirement clean: no live references found` (31534 Python files scanned)
- `PYTHONPATH=src .venv_ep0/bin/python scripts/check_phase5_gate.py` -> `P5.gate GREEN`
- `PYTHONPATH=src .venv_ep0/bin/python scripts/check_phase4_gate.py` -> fast subset tests green (`1107 passed`) but timing gate failed: `101.0 s` vs the 60 s budget; a follow-up `pytest tests/sim -n 16 -q -m "not slow" --durations=25` showed the slowest offenders are pre-existing unmarked long sim tests, led by `test_v32_release_flash_sim_full_main_post_flash_state` at `43.86s`.
- Temp `assemble_v17` rebuild of `src/dlcp_fw/asm/dlcp_control_v173.asm` after RAM `lfsr` alias migration matched the canonical V1.73 HEX file byte-for-byte; V3.4 was rebuilt canonically after the lifecycle range-clear/I2C recovery changes.
...
- `docs/RELEASE_ARCHIVE.md` (historical MAIN/CONTROL release table; root README focuses on V3.5 + V1.73)
...
- `docs/PROPOSAL_1_SRC4382_USB_DIAGNOSTICS_SPEC.md` / `docs/PROPOSAL_1_SRC4382_USB_DIAGNOSTICS_IMPL.md` (V3.5 MAIN USB cmd `0x45` SRC4382 selected-signal diagnostics; V1a implemented, CONTROL/LCD/page-1/raw-window work deferred)
- `docs/SRC4382_USB_DIAGNOSTICS_SPEC.md` (older draft V3.2+ MAIN USB HID endpoints for SRC4382 selected-signal diagnostics)
...
Build the canonical V3.4 release:

```bash
.venv_ep0/bin/python scripts/build_v34_release.py
```

Build the canonical V3.5 release:
...
Build the canonical V1.73 CONTROL release:
...
- Canonical MAIN release output is always `firmware/patched/releases/DLCP_Firmware_V3.4.hex`.
- Each canonical `V3.4` build must increment the EEPROM revision byte in `src/dlcp_fw/asm/dlcp_main_v34.asm` at `eeprom_data[0x82]`, update the boot-time runtime identity literal, update the cmd `0x25` identity revision nibbles, and pass RAM-bank safety for `main-v34`; `scripts/build_v34_release.py` is the required path because it keeps all of those gates in sync before publishing the HEX.
- `scripts/dlcp_v34_release_flash.py` is the canonical operator wrapper for V3.4 MAIN flashing. It preserves the V3.2/V3.3 baked-preset/no-local-captures behavior and forwards to `scripts/dlcp_main_flash.py`.
- V3.4 is a previous MAIN release line; use V3.5 for the current MAIN release line when paired with the V1.73 CONTROL candidate.

## V3.5 Release Ceremony
...
- `scripts/flash_control_safe.sh` previously defaulted to the canonical `V1.72` hex; use `--hex firmware/patched/releases/DLCP_Control_V1.72.hex` when intentionally flashing the previous pair.
...
## V1.73 Release Ceremony

- Canonical CONTROL release output is always `firmware/patched/releases/DLCP_Control_V1.73.hex`.
- Each canonical `V1.73` build must increment the flashed release-metadata byte in `src/dlcp_fw/asm/dlcp_control_v173.asm` at `control_release_metadata[11]`, bake the build date into `control_release_metadata[12..15]` plus the LCD boot splash, and pass RAM-bank safety for `control-v173`; `scripts/build_v173_release.py` is the required path because it updates and verifies those fields before publishing the HEX.
- `scripts/flash_control_safe.sh` defaults to the canonical `V1.73` hex.
- V1.73 is the current CONTROL release line when paired with V3.5 MAIN and remains backward compatible with V3.4/V3.3/V3.2 staged or rollback combinations covered by the V3.4/V3.5 compatibility and identity tests.  The current rev `0x63` PB2 full-table intent persistence + PB1/PB2 persistence + RC5 F4/F5 shortcut hardening + real-RB5 POWER-wake recovery + LCD refresh-budget + asleep-preset host-echo isolation + V3.4+ Diagnostics rev16 parsing + cold-WAITING ISR scratch isolation + test-robustness candidate is non-hardware gated; live PB2 DOWN, audio-routing, persistence, IR, and test-robustness field gates remain required before claiming hardware field closure.
