...
- `.venv_ep0/bin/python scripts/build_v35_release.py` -> canonical `DLCP_Firmware_V3.5.hex` now reports EEPROM tuple `3.5 / rev 0x91` and cmd `0x25` identity rev `0091`.  The previous TBLPTR-safe EEPROM walker bugfix build moved rev `0x008F -> 0x0090` with focused gates: `tests/sim/test_memory_corruption_instrumentation.py` -> `8 passed in 164.26s`; `scripts/memory_corruption_trace.py --expect-clean` -> first violation `none`, corrupt preset-B units `none`; V3.5 release/identity focused tests -> `11 passed in 21.17s`.
