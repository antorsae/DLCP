...
## Patch Strategy
To support `R-L` (Index 4), the binary requires the injection of a new string and patching of 6 unrolled VCL initialization blocks to detour to a Code Cave, where the 5th `TStrings.Add` instruction will be executed. See `docs/analysis/HFD_v2.12-codex.md` and `docs/analysis/HFD_v2.12-RL-binary-patch-plan.md` for the current write-up and patch strategy.
