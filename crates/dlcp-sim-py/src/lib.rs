...
impl Chain {
    ...
    /// level is only *visible* on subsequent PORT reads when
    /// the pin is configured digital (ANSEL/ANSELH bit clear
    /// in refresh_port's analog mask); analog-configured pins
