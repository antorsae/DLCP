...
from dlcp_fw.flash import dlcp_v34_release_flash as release_flash
...
def test_main_left_forwards_canonical_v34_release_args(monkeypatch, tmp_path) -> None:
    release_hex = tmp_path / "DLCP_Firmware_V3.4.hex"
    ...
    monkeypatch.setattr(release_flash, "V34_MAIN_HEX", release_hex)
...
def test_main_missing_local_captures_warns_and_flashes_unbaked_v34_release(
    monkeypatch,
    tmp_path,
    capsys,
) -> None:
...
def test_info_only_passthrough_does_not_require_v34_release_artifacts(monkeypatch) -> None:
    seen: dict[str, list[str]] = {}
...
def test_v34_flash_requires_explicit_route(monkeypatch) -> None:
