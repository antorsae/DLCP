...
def test_info_only_reports_active_preset(monkeypatch, capsys) -> None:
    ...
    monkeypatch.setattr(dlcp_preset, "_probe_active_preset_ep0", lambda vid, pid: "B")
...
def test_switch_sends_cmd20_and_verifies(monkeypatch, capsys) -> None:
    ...
    def _fake_switch(*, vid, pid, preset):
        seen["vid"] = vid
        seen["pid"] = pid
        seen["preset"] = preset
        return {
    ...
    monkeypatch.setattr(dlcp_preset, "_probe_active_preset_ep0", lambda vid, pid: "A")
    flags = iter([0x84, 0x04, 0x04])
    monkeypatch.setattr(dlcp_preset, "_read_active_flags_ep0", lambda vid, pid: next(flags))
...
def test_switch_errors_if_verify_never_reaches_target(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda *, vid, pid, preset: {
            "active_flags_before": 0x08,
            "active_flags_write": 0x8C,
        },
    )
    monkeypatch.setattr(dlcp_preset, "_probe_active_preset_ep0", lambda vid, pid: "A")
    monkeypatch.setattr(dlcp_preset, "_read_active_flags_ep0", lambda vid, pid: 0x80)
...
def test_switch_reports_low_level_switch_error(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        dlcp_preset,
        "_write_preset_switch_ep0",
        lambda *, vid, pid, preset: (_ for _ in ()).throw(
