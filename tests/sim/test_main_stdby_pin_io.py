...
bridge-FIFO UART byte injection.  The behaviour they covered is
exercised end-to-end by the V1.71×V3.2 chain tests in
``test_v171_v32_layer5_diag_chain.py``; the V1.62b OERR-recovery
fix is preserved by the binary-scan regression test below, which
catches any new ``bcf PIE1, TXIE`` instruction that snuck into the
patch stub region.
"""
